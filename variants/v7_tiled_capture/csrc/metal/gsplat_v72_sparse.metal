...
inline bool visible_at_pixel(ProjectedGaussian g,
                             float2 p,
                             constant Uniforms& u,
...
inline uint flatten_tile(uint tx, uint ty, constant Uniforms& u) {
    return ty * u.tiles_x + tx;
}

inline uchar pack_front_meta(uint count, bool overflow) {
    uchar v = uchar(count & 0x0Fu);
    if (overflow) v = uchar(v | 0x80u);
    return v;
}

inline uint unpack_front_count(uchar meta) {
    return uint(meta & 0x0Fu);
}

inline bool unpack_front_overflow(uchar meta) {
    return (meta & 0x80u) != 0;
}
...
kernel void capture_front_k_binned(
    const device ProjectedGaussian* gaussians [[buffer(0)]],
    const device int* tile_offsets [[buffer(1)]],
    const device ushort* tile_ids [[buffer(2)]],
    constant Uniforms& u [[buffer(3)]],
    device ushort* front_ids [[buffer(4)]],
    device float* front_raw_alpha [[buffer(5)]],
    device uchar* front_meta [[buffer(6)]],
    ...
    for (uint j = 0; j < u.front_k; ++j) {
        front_ids[base_k + j] = ushort(0u);
        front_raw_alpha[base_k + j] = 0.0f;
    }
    front_meta[gid] = uchar(0u);

    uint x = gid % u.width;
    uint y = gid / u.width;
    uint tx = x / u.tile_size;
    uint ty = y / u.tile_size;
    uint tile = flatten_tile(tx, ty, u);
    ...
    for (int ptr = start; ptr < end; ++ptr) {
        ushort idx = tile_ids[ptr];
        float raw_alpha = 0.0f;
        float alpha = 0.0f;
        float2 d;
        float power = 0.0f;
        if (!visible_at_pixel(gaussians[idx], p, u, raw_alpha, alpha, d, power)) {
            continue;
        }
        if (captured < u.front_k) {
            front_ids[base_k + captured] = idx;
            front_raw_alpha[base_k + captured] = raw_alpha;
            captured += 1u;
        } else {
            overflow = true;
            break;
    ...
    front_meta[gid] = pack_front_meta(captured, overflow);
}

kernel void backward_front_k(
    const device ProjectedGaussian* gaussians [[buffer(0)]],
    const device float* grad_image [[buffer(1)]],
    const device ushort* front_ids [[buffer(2)]],
    const device float* front_raw_alpha [[buffer(3)]],
    const device uchar* front_meta [[buffer(4)]],
    constant Uniforms& u [[buffer(5)]],
    device atomic_float* g_geom [[buffer(6)]],
    device atomic_float* g_conic [[buffer(7)]],
    device atomic_float* g_color [[buffer(8)]],
    uint gid [[thread_position_in_grid]]) {
    uint P = u.width * u.height;
    if (gid >= P) return;

    uchar meta = front_meta[gid];
    if (unpack_front_overflow(meta)) return;

    int n = int(unpack_front_count(meta));
    if (n <= 0) return;
    if (n > int(kMaxFrontK)) return;

    ushort ids[kMaxFrontK];
...
kernel void backward_overflow_replay_binned(
    const device ProjectedGaussian* gaussians [[buffer(0)]],
    const device float* grad_image [[buffer(1)]],
    const device float* out_image [[buffer(2)]],
    const device int* tile_offsets [[buffer(3)]],
    const device ushort* tile_ids [[buffer(4)]],
    const device uchar* front_meta [[buffer(5)]],
    ...
    uint P = u.width * u.height;
    if (gid >= P) return;
    if (!unpack_front_overflow(front_meta[gid])) return;
