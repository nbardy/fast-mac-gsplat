...
class V9HWTileStateCapabilities:
    mps_available: bool
...
def _native_probe(compile_pipelines: bool, compile_advanced: bool) -> Dict[str, Any]:
    if _C is None:
        return {
            ...
            "native_probe_error": "native extension is not imported; build variants/v9_hw_tile_state_probe first",
        }
...
def render_constant_rgba(height: int, width: int, rgba: RGBA = (0.125, 0.5, 0.875, 1.0)) -> Tensor:
    ...
    if _C is None:
        raise RuntimeError("native extension is not imported; build variants/v9_hw_tile_state_probe first")
...
def run_tile_state_execution_probe(height: int = 32, width: int = 32, tile_size: int = 32) -> Dict[str, Any]:
    ...
    return dict(_C.run_tile_state_execution_probe(int(height), int(width), int(tile_size)))


def estimate_tile_state_memory(
    *,
    height: int = 2160,
    width: int = 3840,
    tile_size: int = 16,
...
def probe_hw_interop(
    *,
    ...
    return V9HWTileStateCapabilities(
