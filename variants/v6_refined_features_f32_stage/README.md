...
`v6_refined_features_f32_stage` feature backend independently from the older
...
- Package: `torch_gsplat_bridge_v6_refined_features_f32_stage`
...
```bash
python3 benchmarks/benchmark_matrix.py --dry-run --include-reduce-baseline --include-stable-baseline --backward
...
- v6_refined_features_f32_stage stages per-chunk feature/color values in
  threadgroup memory. The first pressure-case benchmark regressed fwd+bwd total
  versus `v6_refined_features_f32_reduce`, so this is a negative-result fork,
