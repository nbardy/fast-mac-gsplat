...
ShaderConfig& shader_config() {
  static ShaderConfig cfg = []() {
    ...
    long long approx_threadgroup_bytes =
        4LL * c.fast_cap +                  // shared sorted ids
        4LL * c.chunk * c.feature_cap +     // staged feature/color chunk
        24LL * c.chunk +                    // means, conics, opacities
        4LL * c.simdgroups * c.feature_cap +// feature-gradient partials
        64LL * c.simdgroups;                // scalar/vector reduction scratch
    TORCH_CHECK(
        approx_threadgroup_bytes <= 49152,
        "v6_refined_features_f32_stage estimated threadgroup memory is too high (",
        approx_threadgroup_bytes,
        " bytes). Reduce GSP_CHUNK, GSP_FEATURE_CAP, GSP_FAST_CAP, or GSP_TILE_SIZE.");
    return c;
  }();
  return cfg;
}
...
std::string load_shader_source() {
  ...
  metalPath = [metalPath stringByAppendingPathComponent:@"gsplat_v6_refined_features_f32_stage_kernels.metal"];
  NSError* err = nil;
  NSString* src = [NSString stringWithContentsOfFile:metalPath encoding:NSUTF8StringEncoding error:&err];
  TORCH_CHECK(src != nil, "Failed to read gsplat_v6_refined_features_f32_stage_kernels.metal: ", err.localizedDescription.UTF8String);
...
MetalV6RefinedFeaturesKernels& kernels() {
  ...
  std::call_once(once, []() {
    ...
    out.count_tiles = lib->getKernelFunction("v6_refined_features_f32_stage_count_tiles");
    out.emit_binned_ids = lib->getKernelFunction("v6_refined_features_f32_stage_emit_binned_ids");
    out.tile_fast_forward_eval = lib->getKernelFunction("v6_refined_features_f32_stage_tile_fast_forward_eval");
    out.tile_fast_forward_state = lib->getKernelFunction("v6_refined_features_f32_stage_tile_fast_forward_state");
    out.tile_fast_backward_saved = lib->getKernelFunction("v6_refined_features_f32_stage_tile_fast_backward_saved");
    out.tile_active_forward_eval = lib->getKernelFunction("v6_refined_features_f32_stage_tile_active_forward_eval");
    out.tile_active_forward_state = lib->getKernelFunction("v6_refined_features_f32_stage_tile_active_forward_state");
    out.tile_active_backward_saved = lib->getKernelFunction("v6_refined_features_f32_stage_tile_active_backward_saved");
    out.tile_overflow_forward = lib->getKernelFunction("v6_refined_features_f32_stage_tile_overflow_forward");
    out.tile_overflow_backward = lib->getKernelFunction("v6_refined_features_f32_stage_tile_overflow_backward");
  });
  return out;
}
