...
inline float3 load3_feature_sh(const threadgroup float* base, uint idx) {
  uint b = idx * GSP_FEATURE_CAP;
  return float3(base[b + 0u], base[b + 1u], base[b + 2u]);
}
...
inline void add_weighted_features_sh(
    device float* out_features,
    const threadgroup float* sh_colors,
    uint pix,
    uint local,
    float w,
    const constant MetaI32& mi) {
  uint fdim = feature_dim(mi);
  uint out_base = pix * fdim;
  uint color_base = local * GSP_FEATURE_CAP;
  for (uint f = 0u; f < fdim; ++f) {
    out_features[out_base + f] += w * sh_colors[color_base + f];
  }
}
...
inline float dot_pixel_features_sh(
    const device float* grad_features,
    const threadgroup float* sh_colors,
    uint pix,
    uint local,
  ...
  float out = 0.0f;
  for (uint f = 0u; f < fdim; ++f) {
    out += grad_features[grad_base + f] * sh_colors[color_base + f];
  }
  return out;
}
...
inline void load_chunk_params(
  ...
  for (uint i = tid; i < chunk_n; i += GSP_THREADS) {
    ...
    uint sh_f = i * GSP_FEATURE_CAP;
    uint g_f = g * fdim;
    for (uint f = 0u; f < fdim; ++f) {
      sh_colors[sh_f + f] = colors[g_f + f];
    }
...
kernel void v6_refined_features_f32_stage_count_tiles(
...
kernel void v6_refined_features_f32_stage_emit_binned_ids(
    const device float2* means2d [[buffer(0)]],
    const device float* conics [[buffer(1)]],
...
kernel void v6_refined_features_f32_stage_tile_fast_forward_eval(
  ...
  for (uint chunk_start = 0u; chunk_start < count; chunk_start += GSP_CHUNK) {
    ...
    load_chunk_params(means2d, conics, colors, opacities, mi, shared_ids, chunk_start, chunk_n, tid, sh_means, sh_conics, sh_colors, sh_opacities);
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    if (valid && T > mf.transmittance_threshold) {
      for (uint j = 0u; j < chunk_n; ++j) {
        ...
        add_weighted_features_sh(out_features, sh_colors, pix, j, w, mi);
        T *= (1.0f - alpha);
...
kernel void v6_refined_features_f32_stage_tile_fast_forward_state(
    const device uint* tile_counts [[buffer(0)]],
    const device int* tile_offsets [[buffer(1)]],
...
kernel void v6_refined_features_f32_stage_tile_fast_backward_saved(
      ...
      if (valid && global_i < end_i) {
        ...
        if (eval_alpha(p, m, q.x, q.y, q.z, opacity, mf, alpha, raw_alpha, power, d)) {
          ...
          if (feature3) {
            ...
            float3 c3 = load3_feature_sh(sh_colors, uint(local));
            dot_gc = dot(go3, c3) + alpha_grad;
            if (!skip_color_grad) l_gcol3 = go3 * color_scale;
          } else {
            dot_gc = dot_pixel_features_sh(grad_features, sh_colors, pix, uint(local), mi) + alpha_grad;
          }
...
kernel void v6_refined_features_f32_stage_tile_active_forward_eval(
...
kernel void v6_refined_features_f32_stage_tile_active_forward_state(
    const device uint* active_tile_ids [[buffer(0)]],
    const device uint* tile_counts [[buffer(1)]],
    const device int* tile_offsets [[buffer(2)]],
...
kernel void v6_refined_features_f32_stage_tile_active_backward_saved(
    const device float* grad_features [[buffer(0)]],
    const device float* grad_alpha [[buffer(1)]],
...
kernel void v6_refined_features_f32_stage_tile_overflow_forward(
  ...
  for (uint chunk_start = 0u; chunk_start < count; chunk_start += GSP_CHUNK) {
    ...
    threadgroup_barrier(mem_flags::mem_threadgroup);
    load_chunk_params(means2d, conics, colors, opacities, mi, sh_ids, 0u, chunk_n, tid, sh_means, sh_conics, sh_colors, sh_opacities);
    threadgroup_barrier(mem_flags::mem_threadgroup);
    uint alive_total = reduce_alive((valid && T > mf.transmittance_threshold) ? 1u : 0u, simd_lane, simd_group, tg_alive);
    if (alive_total == 0u) break;
    if (valid && T > mf.transmittance_threshold) {
      for (uint j = 0u; j < chunk_n; ++j) {
        ...
        if (!eval_alpha(p, m, q.x, q.y, q.z, sh_opacities[j], mf, alpha, raw_alpha, power, d)) continue;
        float w = T * alpha;
        add_weighted_features_sh(out_tiles, sh_colors, pix, j, w, mi);
        T *= (1.0f - alpha);
        if (T <= mf.transmittance_threshold) break;
      }
    }
...
kernel void v6_refined_features_f32_stage_tile_overflow_backward(
      ...
      if (valid && global_i < end_i) {
        ...
        if (eval_alpha(p, m, q.x, q.y, q.z, opacity, mf, alpha, raw_alpha, power, d)) {
          ...
          if (feature3) {
            ...
            dot_gc = dot_pixel_features_sh(grad_tiles, sh_colors, pix, uint(local), mi) + alpha_grad;
          }
          float g_alpha = T_prev * (dot_gc - gT);
          float gate = (raw_alpha < mf.max_alpha) ? 1.0f : 0.0f;
          float g_raw = g_alpha * gate;
