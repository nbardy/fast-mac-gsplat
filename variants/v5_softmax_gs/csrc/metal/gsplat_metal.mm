...
ShaderConfig& shader_config() {
  static ShaderConfig cfg = []() {
    ...
    c.tape_cap = env_int("GSP_TAPE_CAP", 8);
    TORCH_CHECK(c.tape_cap > 0 && c.tape_cap <= 32, "GSP_TAPE_CAP must be in [1, 32]; got ", c.tape_cap);
...
std::string load_shader_source() {
  ...
  metalPath = [metalPath stringByAppendingPathComponent:@"gsplat_v5_softmax_gs_kernels.metal"];
  NSError* err = nil;
  NSString* src = [NSString stringWithContentsOfFile:metalPath encoding:NSUTF8StringEncoding error:&err];
  TORCH_CHECK(src != nil, "Failed to read gsplat_v5_softmax_gs_kernels.metal: ", err.localizedDescription.UTF8String);
  ...
  preamble += "#define GSP_TAPE_CAP " + std::to_string(cfg.tape_cap) + "u\n";
...
struct MetalV5SoftmaxGSKernels {
  ...
  std::shared_ptr<MetalKernelFunction> tile_fast_backward_softmax_recompute;
  std::shared_ptr<MetalKernelFunction> tile_fast_softmax_bounded_tape;
  std::shared_ptr<MetalKernelFunction> softmax_tape_color_backward;
  std::shared_ptr<MetalKernelFunction> softmax_tape_scalar_backward;
  std::shared_ptr<MetalKernelFunction> tile_overflow_forward;
  std::shared_ptr<MetalKernelFunction> tile_overflow_backward;
  std::shared_ptr<MetalKernelFunction> tile_overflow_backward_softmax_recompute;
  std::shared_ptr<MetalKernelFunction> tile_overflow_softmax_bounded_tape;
};

MetalV5SoftmaxGSKernels& kernels() {
  static std::once_flag once;
  static std::unique_ptr<DynamicMetalShaderLibrary> lib;
  static MetalV5SoftmaxGSKernels out;
  std::call_once(once, []() {
    ...
    out.tile_fast_backward_softmax_recompute = lib->getKernelFunction("tile_fast_backward_softmax_recompute");
    out.tile_fast_softmax_bounded_tape = lib->getKernelFunction("tile_fast_softmax_bounded_tape");
    out.softmax_tape_color_backward = lib->getKernelFunction("softmax_tape_color_backward");
    out.softmax_tape_scalar_backward = lib->getKernelFunction("softmax_tape_scalar_backward");
    ...
    out.tile_overflow_backward_softmax_recompute = lib->getKernelFunction("tile_overflow_backward_softmax_recompute");
    out.tile_overflow_softmax_bounded_tape = lib->getKernelFunction("tile_overflow_softmax_bounded_tape");
  });
  return out;
}
...
void check_tape_inputs(
  ...
  TORCH_CHECK(conics.dim() == 2 && conics.size(1) == 3, "conics must be [BG,3]");
  TORCH_CHECK(opacities.dim() == 1, "opacities must be [BG]");
  TORCH_CHECK(means2d.size(0) == conics.size(0) && means2d.size(0) == opacities.size(0),
              "All tape inputs must agree on flattened BG");
}
...
void check_depths(const torch::Tensor& depths, int64_t expected_count) {
  ...
  TORCH_CHECK(depths.dim() == 1, "depths must be [BG]");
  TORCH_CHECK(depths.size(0) == expected_count, "depths must match flattened BG");
}
...
torch::Tensor metal_render_fast_forward_eval(
  ...
  check_inputs(means2d, conics, colors, opacities);
  check_depths(depths, means2d.size(0));
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> metal_render_fast_backward_softmax_recompute(
  ...
  auto g_colors = torch::zeros_like(colors, opts_f);
  auto g_opacities = torch::zeros_like(opacities, opts_f);
  auto g_depths = torch::zeros_like(depths, opts_f);

  if (meta.tile_count > 0) {
    launch(k.tile_fast_backward_softmax_recompute, [&](MetalKernelFunction& fn) {
      fn.setArg(0, grad_out.contiguous());
      fn.setArg(1, tile_counts);
      fn.setArg(2, tile_offsets);
      fn.setArg(3, binned_ids);
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> metal_render_fast_softmax_bounded_tape(
    ...
    const torch::Tensor& tile_offsets,
    const torch::Tensor& binned_ids) {
  check_tape_inputs(means2d, conics, opacities);
  check_depths(depths, means2d.size(0));
  check_aux_i32(tile_counts, "tile_counts");
  check_aux_i32(tile_offsets, "tile_offsets");
  check_aux_i32(binned_ids, "binned_ids");
  ...
  auto meta = parse_meta(meta_i32, meta_f32);
  TORCH_CHECK(meta.softmax_tape_k > 0, "softmax_tape_k must be positive");
  auto& sc = shader_config();
  TORCH_CHECK(meta.softmax_tape_k <= sc.tape_cap, "softmax_tape_k exceeds shader tape cap ", sc.tape_cap);
  auto& k = kernels();
  auto opts_f = means2d.options().dtype(torch::kFloat32);
  auto opts_i32 = means2d.options().dtype(torch::kInt32);

  auto selected_ids = torch::full({meta.batch_size, meta.height, meta.width, meta.softmax_tape_k}, -1, opts_i32);
  auto selected_weights = torch::zeros({meta.batch_size, meta.height, meta.width, meta.softmax_tape_k}, opts_f);
  auto residual_weight = torch::zeros({meta.batch_size, meta.height, meta.width}, opts_f);
  auto final_alpha = torch::zeros({meta.batch_size, meta.height, meta.width}, opts_f);

  if (meta.tile_count > 0) {
    launch(k.tile_fast_softmax_bounded_tape, [&](MetalKernelFunction& fn) {
      fn.setArg(0, tile_counts);
      fn.setArg(1, tile_offsets);
      fn.setArg(2, binned_ids);
      fn.setArg(3, means2d);
      fn.setArg(4, conics);
      fn.setArg(5, opacities);
      fn.setArg(6, depths);
      fn.setArg(7, meta_i32);
      fn.setArg(8, meta_f32);
      fn.setArg(9, selected_ids);
      fn.setArg(10, selected_weights);
      fn.setArg(11, residual_weight);
      fn.setArg(12, final_alpha);
      fn.dispatch((uint64_t)meta.tile_count * (uint64_t)sc.threads, (uint64_t)sc.threads);
    });
  }
  return std::make_tuple(selected_ids, selected_weights, residual_weight, final_alpha);
}
...
torch::Tensor metal_render_softmax_tape_color_backward(
    ...
    const torch::Tensor& meta_f32) {
  TORCH_CHECK(grad_out.device().is_mps(), "grad_out must be on MPS");
  check_aux_i32(selected_ids, "selected_ids");
  TORCH_CHECK(selected_weights.device().is_mps(), "selected_weights must be on MPS");
  TORCH_CHECK(selected_weights.scalar_type() == torch::kFloat32, "selected_weights must be float32");

  auto meta = parse_meta(meta_i32, meta_f32);
  TORCH_CHECK(meta.softmax_tape_k > 0, "softmax_tape_k must be positive");
  TORCH_CHECK(grad_out.dim() == 4 && grad_out.size(0) == meta.batch_size && grad_out.size(1) == meta.height &&
                  grad_out.size(2) == meta.width && grad_out.size(3) == 3,
              "grad_out must be [B,H,W,3]");
  TORCH_CHECK(selected_ids.dim() == 4 && selected_ids.size(0) == meta.batch_size && selected_ids.size(1) == meta.height &&
                  selected_ids.size(2) == meta.width && selected_ids.size(3) == meta.softmax_tape_k,
              "selected_ids must be [B,H,W,K]");
  TORCH_CHECK(selected_weights.sizes() == selected_ids.sizes(), "selected_weights must match selected_ids shape");

  auto& k = kernels();
  auto opts_f = grad_out.options().dtype(torch::kFloat32);
  auto g_colors = torch::zeros({meta.gaussians, 3}, opts_f);
  uint64_t total = (uint64_t)meta.batch_size * (uint64_t)meta.height * (uint64_t)meta.width * (uint64_t)meta.softmax_tape_k;
  if (total > 0) {
    launch(k.softmax_tape_color_backward, [&](MetalKernelFunction& fn) {
      fn.setArg(0, grad_out.contiguous());
      fn.setArg(1, selected_ids);
      fn.setArg(2, selected_weights);
      fn.setArg(3, meta_i32);
      fn.setArg(4, g_colors);
      fn.dispatch(total, 256);
    });
  }
  return g_colors;
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> metal_render_softmax_tape_scalar_backward(
  ...
  auto& k = kernels();
  auto opts_f = means2d.options().dtype(torch::kFloat32);
  auto g_means2d = torch::zeros({meta.gaussians, 2}, opts_f);
  auto g_conics = torch::zeros({meta.gaussians, 3}, opts_f);
  auto g_opacities = torch::zeros({meta.gaussians}, opts_f);
  auto g_depths = torch::zeros({meta.gaussians}, opts_f);
  uint64_t total_pixels = (uint64_t)meta.batch_size * (uint64_t)meta.height * (uint64_t)meta.width;
  if (total_pixels > 0) {
    launch(k.softmax_tape_scalar_backward, [&](MetalKernelFunction& fn) {
      fn.setArg(0, grad_out.contiguous());
      fn.setArg(1, selected_ids);
      fn.setArg(2, means2d);
      fn.setArg(3, conics);
      ...
      fn.setArg(7, meta_i32);
      fn.setArg(8, meta_f32);
      fn.setArg(9, g_means2d);
      fn.setArg(10, g_conics);
      fn.setArg(11, g_opacities);
      fn.setArg(12, g_depths);
      fn.dispatch(total_pixels, 256);
    });
  }
  return std::make_tuple(g_means2d, g_conics, g_opacities, g_depths);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> metal_render_overflow_backward_softmax_recompute(
  ...
  const int64_t To = overflow_tile_ids.size(0);
  if (To > 0) {
    launch(k.tile_overflow_backward_softmax_recompute, [&](MetalKernelFunction& fn) {
      fn.setArg(0, grad_tiles.contiguous());
      fn.setArg(1, overflow_tile_ids);
      fn.setArg(2, overflow_tile_offsets);
      fn.setArg(3, overflow_sorted_ids);
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> metal_render_overflow_softmax_bounded_tape(
  ...
  const int64_t To = overflow_tile_ids.size(0);
  auto selected_ids = torch::full({To, meta.tile_size, meta.tile_size, meta.softmax_tape_k}, -1, opts_i32);
  auto selected_weights = torch::zeros({To, meta.tile_size, meta.tile_size, meta.softmax_tape_k}, opts_f);
  auto residual_weight = torch::zeros({To, meta.tile_size, meta.tile_size}, opts_f);
  auto final_alpha = torch::zeros({To, meta.tile_size, meta.tile_size}, opts_f);

  if (To > 0) {
    launch(k.tile_overflow_softmax_bounded_tape, [&](MetalKernelFunction& fn) {
      fn.setArg(0, overflow_tile_ids);
      fn.setArg(1, overflow_tile_offsets);
      fn.setArg(2, overflow_sorted_ids);
      fn.setArg(3, means2d);
