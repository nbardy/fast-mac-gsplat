...
_BACKWARD_STATE_MODES = ("compute", "tile_stop", "final_T", "pixel_stop")
_HARDWARE_TRAIN_UNSUPPORTED_REASON = (
    "hardware training render path is unsupported: GPU-native Metal render output interop with Torch/MPS "
    "is not implemented, and exact hardware capture of the forward processed prefix M is not implemented"
)


@dataclass(frozen=True)
class HardwareTrainStatePlan:
    batch_size: int
    height: int
    width: int
    tile_size: int
    ...
    requested_capture_modes: Tuple[str, ...]
    selected_capture_modes: Tuple[str, ...]
    tile_stop_shape: Tuple[int, int, int]
    final_T_shape: Tuple[int, int, int]
    pixel_stop_shape: Tuple[int, int, int]
    tile_stop_bytes: int
    final_T_bytes: int
    pixel_stop_bytes: int
    requested_state_bytes: int
    selected_state_bytes: int
    v8_compute_tile_stop_bytes: int
    allocates_per_pixel_state_by_default: bool = False


@dataclass(frozen=True)
class HardwareTrainStateBuffers:
    """Future hardware state container; defaults intentionally allocate nothing."""

    tile_stop: Optional[Tensor] = None
    final_T: Optional[Tensor] = None
    pixel_stop: Optional[Tensor] = None


@dataclass(frozen=True)
class HardwareTrainProbe:
    requested: bool
    available: bool
    selected: bool
    policy: str
    backward_state_mode: str
    reason: str
    requested_state_mode: str = "compute"
    selected_state_mode: str = "compute"
    requested_capture_modes: Tuple[str, ...] = ()
    selected_capture_modes: Tuple[str, ...] = ()
    fallback_reason: str = ""
    native_namespace: str = "gsplat_metal_v8_hw_train"
    avoids_cpu_readback: bool = True
...
class RasterConfig:
    ...
    use_hardware_train: bool = False
    hardware_train_policy: str = "fallback"  # fallback | strict
    capture_final_T: bool = False
    capture_stop_count: bool = True
    capture_pixel_stop: bool = False
    backward_state_mode: str = "compute"  # compute | tile_stop | final_T | pixel_stop


_WARNED_HARDWARE_TRAIN_REASONS: Set[str] = set()


def _capture_modes_from_config(config: RasterConfig) -> Tuple[str, ...]:
    modes = []
    if config.capture_stop_count:
        modes.append("tile_stop")
    if config.capture_final_T:
        modes.append("final_T")
    if config.capture_pixel_stop:
        modes.append("pixel_stop")
    return tuple(modes)


def _capture_modes_for_state_mode(mode: str) -> Tuple[str, ...]:
    if mode == "compute":
        return ()
    if mode == "tile_stop":
        return ("tile_stop",)
    if mode == "final_T":
        return ("tile_stop", "final_T")
    if mode == "pixel_stop":
        return ("tile_stop", "pixel_stop")
    raise ValueError(f"unsupported hardware-training state mode: {mode}")


def _state_mode_bytes(mode: str, *, tile_stop_bytes: int, final_T_bytes: int, pixel_stop_bytes: int) -> int:
    if mode == "compute":
        return 0
    if mode == "tile_stop":
        return int(tile_stop_bytes)
    if mode == "final_T":
        return int(tile_stop_bytes + final_T_bytes)
    if mode == "pixel_stop":
        return int(tile_stop_bytes + pixel_stop_bytes)
    raise ValueError(f"unsupported hardware-training state mode: {mode}")


def _validate_hardware_state_config(config: RasterConfig) -> None:
    mode = config.backward_state_mode
    if mode == "compute":
        if config.capture_final_T or config.capture_pixel_stop:
            raise ValueError(
                "backward_state_mode='compute' cannot request capture_final_T or capture_pixel_stop; "
                "use backward_state_mode='final_T' or 'pixel_stop' for those per-pixel state captures"
            )
        return

    if not config.capture_stop_count:
        raise ValueError(f"backward_state_mode={mode!r} requires capture_stop_count=True")

    if mode == "tile_stop":
        if config.capture_final_T or config.capture_pixel_stop:
            raise ValueError(
                "backward_state_mode='tile_stop' is only compatible with capture_stop_count=True; "
                "disable capture_final_T/capture_pixel_stop or select the matching state mode"
            )
    elif mode == "final_T":
        if not config.capture_final_T:
            raise ValueError("backward_state_mode='final_T' requires capture_final_T=True")
        if config.capture_pixel_stop:
            raise ValueError("capture_final_T and capture_pixel_stop are mutually exclusive in this scaffold")
    elif mode == "pixel_stop":
        if not config.capture_pixel_stop:
            raise ValueError("backward_state_mode='pixel_stop' requires capture_pixel_stop=True")
        if config.capture_final_T:
            raise ValueError("capture_pixel_stop and capture_final_T are mutually exclusive in this scaffold")


def _make_hardware_train_state_plan(
    config: RasterConfig,
    *,
    batch_size: int,
    selected_state_mode: str,
    ...
    if selected_state_mode not in _BACKWARD_STATE_MODES:
        raise ValueError(f"selected_state_mode must be one of: {', '.join(_BACKWARD_STATE_MODES)}")

    tiles_y = (config.height + config.tile_size - 1) // config.tile_size
    tiles_x = (config.width + config.tile_size - 1) // config.tile_size
    tile_count = int(batch_size * tiles_y * tiles_x)
    pixel_count = int(batch_size * config.height * config.width)
    tile_stop_bytes = tile_count * 4
    final_T_bytes = pixel_count * 4
    pixel_stop_bytes = pixel_count * 4

    requested_state_mode = config.backward_state_mode
    return HardwareTrainStatePlan(
        ...
        requested_state_mode=requested_state_mode,
        selected_state_mode=selected_state_mode,
        requested_capture_modes=_capture_modes_for_state_mode(requested_state_mode),
        selected_capture_modes=_capture_modes_for_state_mode(selected_state_mode),
        tile_stop_shape=(int(batch_size), int(tiles_y), int(tiles_x)),
        final_T_shape=(int(batch_size), int(config.height), int(config.width)),
        pixel_stop_shape=(int(batch_size), int(config.height), int(config.width)),
        tile_stop_bytes=tile_stop_bytes,
        final_T_bytes=final_T_bytes,
        pixel_stop_bytes=pixel_stop_bytes,
        requested_state_bytes=_state_mode_bytes(
            requested_state_mode,
            tile_stop_bytes=tile_stop_bytes,
            final_T_bytes=final_T_bytes,
            pixel_stop_bytes=pixel_stop_bytes,
        ),
        selected_state_bytes=_state_mode_bytes(
            selected_state_mode,
            tile_stop_bytes=tile_stop_bytes,
            final_T_bytes=final_T_bytes,
            pixel_stop_bytes=pixel_stop_bytes,
        ),
        v8_compute_tile_stop_bytes=tile_stop_bytes,
    )


def estimate_hardware_train_state(
    config: RasterConfig,
    *,
    batch_size: int = 1,
    selected_state_mode: Optional[str] = None,
) -> HardwareTrainStatePlan:
    """Plan hardware-training state memory without allocating per-pixel buffers."""
    _runtime_validate(config)
    selected = config.backward_state_mode if selected_state_mode is None else selected_state_mode
    return _make_hardware_train_state_plan(config, batch_size=batch_size, selected_state_mode=selected)


def _state_plan_profile(plan: HardwareTrainStatePlan) -> Dict[str, Any]:
    return {
        "hardware_train_state_buffer_bytes": {
            "tile_stop": int(plan.tile_stop_bytes),
            "final_T": int(plan.final_T_bytes),
            "pixel_stop": int(plan.pixel_stop_bytes),
        },
        "hardware_train_state_mode_bytes": {
            "compute": 0,
            "tile_stop": int(plan.tile_stop_bytes),
            "final_T": int(plan.tile_stop_bytes + plan.final_T_bytes),
            "pixel_stop": int(plan.tile_stop_bytes + plan.pixel_stop_bytes),
        },
        "hardware_train_requested_state_bytes": int(plan.requested_state_bytes),
        "hardware_train_selected_state_bytes": int(plan.selected_state_bytes),
        "hardware_train_v8_compute_tile_stop_bytes": int(plan.v8_compute_tile_stop_bytes),
        "hardware_train_state_shapes": {
            "tile_stop": tuple(int(x) for x in plan.tile_stop_shape),
            "final_T": tuple(int(x) for x in plan.final_T_shape),
            "pixel_stop": tuple(int(x) for x in plan.pixel_stop_shape),
        },
        "hardware_train_allocates_per_pixel_state_by_default": bool(plan.allocates_per_pixel_state_by_default),
    }
...
def _runtime_validate(config: RasterConfig) -> RuntimeShaderConfig:
    ...
    if config.hardware_train_policy not in ("fallback", "strict"):
        raise ValueError("hardware_train_policy must be one of: fallback, strict")
    if config.backward_state_mode not in _BACKWARD_STATE_MODES:
        raise ValueError("backward_state_mode must be one of: compute, tile_stop, final_T, pixel_stop")
    _validate_hardware_state_config(config)
    return rt


def probe_hardware_train(config: RasterConfig, *, device: Optional[torch.device] = None) -> HardwareTrainProbe:
    """Return the hardware-training capability decision without touching tensor data."""
    _runtime_validate(config)
    requested_capture_modes = _capture_modes_for_state_mode(config.backward_state_mode)
    if not config.use_hardware_train:
        return HardwareTrainProbe(
            requested=False,
            available=False,
            selected=False,
            policy=config.hardware_train_policy,
            backward_state_mode=config.backward_state_mode,
            reason="hardware training disabled; using the v8 compute fallback",
            requested_state_mode=config.backward_state_mode,
            selected_state_mode="compute",
            requested_capture_modes=requested_capture_modes,
            selected_capture_modes=(),
            fallback_reason="",
        )

    if device is not None and device.type != "mps":
        fallback_reason = f"hardware training requires MPS tensors, got device type {device.type!r}"
        return HardwareTrainProbe(
            requested=True,
            available=False,
            selected=False,
            policy=config.hardware_train_policy,
            backward_state_mode=config.backward_state_mode,
            reason=f"{fallback_reason}; using the v8 compute fallback",
            requested_state_mode=config.backward_state_mode,
            selected_state_mode="compute",
            requested_capture_modes=requested_capture_modes,
            selected_capture_modes=(),
    ...
    capture_text = ",".join(requested_capture_modes) if requested_capture_modes else "none"
    configured_capture_text = ",".join(_capture_modes_from_config(config)) or "none"
    return HardwareTrainProbe(
        requested=True,
        available=False,
        selected=False,
        ...
        backward_state_mode=config.backward_state_mode,
        reason=(
            f"{_HARDWARE_TRAIN_UNSUPPORTED_REASON} "
            f"(requested_state_mode={config.backward_state_mode}, selected_state_mode=compute, "
            f"requested_captures={capture_text}, configured_captures={configured_capture_text}); "
            "using the v8 compute fallback"
        ),
        requested_state_mode=config.backward_state_mode,
        selected_state_mode="compute",
        requested_capture_modes=requested_capture_modes,
        selected_capture_modes=(),
        fallback_reason=_HARDWARE_TRAIN_UNSUPPORTED_REASON,
    )


def _handle_hardware_train_request(config: RasterConfig, device: torch.device) -> HardwareTrainProbe:
    status = probe_hardware_train(config, device=device)
    if status.requested and not status.selected:
        if config.hardware_train_policy == "strict":
            raise RuntimeError(status.reason)
        if status.reason not in _WARNED_HARDWARE_TRAIN_REASONS:
            warnings.warn(status.reason, RuntimeWarning, stacklevel=3)
            _WARNED_HARDWARE_TRAIN_REASONS.add(status.reason)
    return status
...
def _effective_active_policy(config: RasterConfig) -> str:
    # Preserve the inherited API: RasterConfig(use_active_tiles=True) still means "on".
    if config.active_policy == "off" and config.use_active_tiles:
        return "on"
    return config.active_policy


def _make_meta(config: RasterConfig, device: torch.device, batch_size: int, gaussians_per_batch: int) -> MetaBundle:
    _runtime_validate(config)
    tiles_y = (config.height + config.tile_size - 1) // config.tile_size
    tiles_x = (config.width + config.tile_size - 1) // config.tile_size
    ...
    gpu_i32 = torch.tensor(
        i32_values,
        device=device,
        dtype=torch.int32,
    )
    gpu_f32 = torch.tensor(
        f32_values,
        device=device,
        dtype=torch.float32,
    )
    return MetaBundle(gpu_i32=gpu_i32, gpu_f32=gpu_f32, host_i32=host_i32, host_f32=host_f32)
...
def _check_inputs(means2d: Tensor, conics: Tensor, colors: Tensor, opacities: Tensor, depths: Tensor) -> None:
    ...
    if means2d.device.type != "mps":
        raise ValueError("v8 fast path requires MPS tensors")
...
def _choose_batch_slices(
    ...
    *,
    ...
    for b0, b1 in base_slices:
        ...
        meta = _make_meta(config, m.device, b1 - b0, G)
        tile_counts, _, _ = torch.ops.gsplat_metal_v8_hw_train.bin(
...
class _RasterizeProjectedGaussiansV8HwTrain(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means2d_b: Tensor,
        ...
        if not hasattr(torch.ops, "gsplat_metal_v8_hw_train"):
            raise RuntimeError("gsplat_metal_v8_hw_train custom ops not found. Build the extension first.")
        ...
        tile_counts, tile_offsets, binned_ids = torch.ops.gsplat_metal_v8_hw_train.bin(
        ...
        resolved_use_active_tiles = _resolve_active_tile_use(
            ...
            total_tiles=int(meta_host_i32[6].item()),
        )
        ...
        if resolved_use_active_tiles:
            ...
            out_fast, tile_stop_counts = torch.ops.gsplat_metal_v8_hw_train.render_active_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
            ...
            out_fast, tile_stop_counts = torch.ops.gsplat_metal_v8_hw_train.render_fast_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
        ...
        if enable_overflow_fallback:
            ...
            if overflow_tile_ids.numel() > 0:
                overflow_tile_imgs = torch.ops.gsplat_metal_v8_hw_train.render_overflow_forward(
                    means_flat,
                    conics_flat,
                    colors_flat,
                    opacities_flat,
    ...
    def backward(ctx, grad_out: Tensor):
        ...
        if ctx.use_active_tiles:
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v8_hw_train.render_active_backward_saved(
                grad_fast,
                means_flat,
                conics_flat,
                colors_flat,
            ...
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v8_hw_train.render_fast_backward_saved(
                grad_fast,
                means_flat,
                conics_flat,
                colors_flat,
        ...
        if ctx.enable_overflow_fallback and overflow_tile_ids.numel() > 0:
            ...
            go_means, go_conics, go_colors, go_opacities = torch.ops.gsplat_metal_v8_hw_train.render_overflow_backward(
                grad_tiles,
                means_flat,
                conics_flat,
                colors_flat,
        ...
        return g_means_b, g_conics_b, g_colors_b, g_opacities_b, g_depths_b, None, None, None, None, None, None, None, None
...
def _rasterize_chunk_eval(
    ...
    B, G = means2d_b.shape[:2]
    meta = _make_meta(config, means2d_b.device, B, G)
    perm = torch.argsort(depths_b.detach(), dim=1, stable=True)
    means2d_s = _batched_gather_2d(means2d_b, perm).contiguous()
    conics_s = _batched_gather_2d(conics_b, perm).contiguous()
    colors_s = _batched_gather_2d(colors_b, perm).contiguous()
    ...
    max_fast_pairs = int(meta.host_i32[7].item())

    resolved_use_active_tiles = _resolve_active_tile_use(
        policy=_effective_active_policy(config),
        tile_counts=tile_counts,
        max_fast_pairs=max_fast_pairs,
        total_tiles=int(meta.host_i32[6].item()),
    )
    ...
    if resolved_use_active_tiles:
        ...
        out_fast = torch.ops.gsplat_metal_v8_hw_train.render_active_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
        ...
        out_fast = torch.ops.gsplat_metal_v8_hw_train.render_fast_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
    ...
    if config.enable_overflow_fallback:
        ...
        if overflow_tile_ids.numel() > 0:
            ...
            out = out_fast.clone()
            _scatter_tile_images_(
                out,
                overflow_tile_ids,
                overflow_tile_imgs,
                int(meta.host_i32[10].item()),
                int(meta.host_i32[3].item()),
                int(meta.host_i32[4].item()),
            )
            return out
    else:
        if bool((tile_counts > max_fast_pairs).any().item()):
...
def _rasterize_batched(
    ...
    _handle_hardware_train_request(config, means2d_b.device)
    B, G = means2d_b.shape[:2]
    tiles_y = (config.height + config.tile_size - 1) // config.tile_size
    tiles_x = (config.width + config.tile_size - 1) // config.tile_size
    ...
    for b0, b1 in batch_slices:
        ...
        if train_mode:
            meta = _make_meta(config, m.device, b1 - b0, G)
            outs.append(
                _RasterizeProjectedGaussiansV8HwTrain.apply(
                    m,
                    q,
                    c,
                    o,
...
def profile_projected_gaussians(
    ...
    *,
    ...
    B, G = means2d_b.shape[:2]
    _runtime_validate(config)
    hw_train_status = _handle_hardware_train_request(config, means2d_b.device)
    hw_train_plan = _make_hardware_train_state_plan(
        config,
        batch_size=B,
        selected_state_mode=hw_train_status.selected_state_mode,
    )
    ...
    for b0, b1 in batch_slices:
        ...
        meta = _make_meta(config, means2d_b.device, b1 - b0, G)
        tile_counts, tile_offsets, binned_ids = torch.ops.gsplat_metal_v8_hw_train.bin(
        ...
        if run_forward or return_image:
            ...
            if resolved_use_active_tiles:
                _, stop_counts = torch.ops.gsplat_metal_v8_hw_train.render_active_forward_state(
                    m_s,
                    q_s,
                    c_s,
                    o_s,
                ...
                _, stop_counts = torch.ops.gsplat_metal_v8_hw_train.render_fast_forward_state(
                    m_s,
                    q_s,
                    c_s,
                    o_s,
    ...
    stats: Dict[str, Any] = {
        ...
        "use_hardware_train": bool(config.use_hardware_train),
        "hardware_train_requested": bool(hw_train_status.requested),
        "hardware_train_policy": config.hardware_train_policy,
        "hardware_train_selected": bool(hw_train_status.selected),
        "hardware_train_available": bool(hw_train_status.available),
        "hardware_train_reason": hw_train_status.reason,
        "hardware_train_fallback_reason": hw_train_status.fallback_reason,
        "hardware_train_requested_state_mode": hw_train_status.requested_state_mode,
        "hardware_train_selected_state_mode": hw_train_status.selected_state_mode,
        "hardware_train_requested_capture_modes": list(hw_train_status.requested_capture_modes),
        "hardware_train_selected_capture_modes": list(hw_train_status.selected_capture_modes),
        "capture_final_T": bool(config.capture_final_T),
        "capture_stop_count": bool(config.capture_stop_count),
        "capture_pixel_stop": bool(config.capture_pixel_stop),
        "backward_state_mode": config.backward_state_mode,
    }
    stats.update(_state_plan_profile(hw_train_plan))
