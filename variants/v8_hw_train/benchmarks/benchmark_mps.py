...
from torch_gsplat_bridge_v8_hw_train import (
    RasterConfig,
...
def main():
    ...
    p.add_argument("--hardware-train", action=argparse.BooleanOptionalAction, default=False)
    p.add_argument("--hardware-train-policy", type=str, default="fallback", choices=("fallback", "strict"))
    p.add_argument("--capture-final-T", action=argparse.BooleanOptionalAction, default=False)
    p.add_argument("--capture-stop-count", action=argparse.BooleanOptionalAction, default=True)
    p.add_argument("--capture-pixel-stop", action=argparse.BooleanOptionalAction, default=False)
    p.add_argument("--backward-state-mode", type=str, default="compute", choices=("compute", "tile_stop", "final_T", "pixel_stop"))
    ...
    cfg = RasterConfig(
        ...
        use_hardware_train=args.hardware_train,
        hardware_train_policy=args.hardware_train_policy,
        capture_final_T=args.capture_final_T,
        capture_stop_count=args.capture_stop_count,
        capture_pixel_stop=args.capture_pixel_stop,
        backward_state_mode=args.backward_state_mode,
    )
    ...
    result = {
        ...
        "use_hardware_train": bool(args.hardware_train),
        "hardware_train_policy": args.hardware_train_policy,
        "capture_final_T": bool(args.capture_final_T),
        "capture_stop_count": bool(args.capture_stop_count),
        "capture_pixel_stop": bool(args.capture_pixel_stop),
        "backward_state_mode": args.backward_state_mode,
