...
try:
    from .world_tube import make_pinhole_world_tube_demo, project_world_tubes_pinhole
...
def main() -> None:
    ...
    row: dict[str, object] = {
        ...
        "depth0": [float(value) for value in depth0.tolist()],
    }
    if torch.backends.mps.is_available():
        ...
        if result.stats.max_rgb_error is None or result.stats.max_rgb_error > 2.0e-5:
            raise AssertionError(f"pinhole projection Metal parity failed: {result.stats.max_rgb_error}")
