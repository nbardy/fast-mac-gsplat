...
def _check_inputs(means2d: Tensor, conics: Tensor, colors: Tensor, opacities: Tensor, depths: Tensor) -> None:
    ...
    if means2d.device.type != "mps":
        raise ValueError("v6_refined_features_f32_gradcache Metal rasterizer inputs must be on MPS")
...
class _RasterizeProjectedGaussiansV6RefinedFeatures(torch.autograd.Function):
    ...
    def forward(
        ...
        if not hasattr(torch.ops, "gsplat_metal_v6_refined_features_f32_gradcache"):
            raise RuntimeError("gsplat_metal_v6_refined_features_f32_gradcache custom ops not found. Build the extension first.")
        ...
        tile_counts, tile_offsets, binned_ids = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.bin(
        ...
        if use_active_tiles:
            ...
            out_fast, alpha_fast, tile_stop_counts = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_active_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
            ...
            out_fast, alpha_fast, tile_stop_counts = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_fast_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
        ...
        if enable_overflow_fallback:
            ...
            if overflow_tile_ids.numel() > 0:
                overflow_tile_imgs, overflow_tile_alpha = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_overflow_forward(
                    means_flat,
                    conics_flat,
                    colors_flat,
                    opacities_flat,
    ...
    def backward(ctx, grad_features: Tensor | None, grad_alpha: Tensor | None):
        ...
        if ctx.use_active_tiles:
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_active_backward_saved(
                grad_fast,
                grad_alpha_fast,
                means_flat,
                conics_flat,
            ...
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_fast_backward_saved(
                grad_fast,
                grad_alpha_fast,
                means_flat,
                conics_flat,
        ...
        if has_overflow:
            ...
            go_means, go_conics, go_colors, go_opacities = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_overflow_backward(
                grad_tiles,
                grad_alpha_tiles,
                means_flat,
                conics_flat,
...
def _rasterize_chunk_eval(
    ...
    if use_active_tiles:
        ...
        out_fast, alpha_fast = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_active_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
        ...
        out_fast, alpha_fast = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_fast_forward_eval(
    ...
    *,
    ...
    for b0 in range(0, B, chunk_b):
        ...
        if run_forward or return_image:
            ...
            if use_active_tiles:
                _, _alpha, stop_counts = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_active_forward_state(
                    m_s, q_s, c_s, o_s, meta_i32, meta_f32, binned_ids, active_tile_ids, tile_counts, tile_offsets
                )
            else:
                _, _alpha, stop_counts = torch.ops.gsplat_metal_v6_refined_features_f32_gradcache.render_fast_forward_state(
