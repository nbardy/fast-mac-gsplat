...
BENCHMARKS = {
    "f32_gradcache_zero_bg": THIS_BENCH,
    "f32_accum": F32_ACCUM_BENCH,
    "f64_accum64": F64_ACCUM64_BENCH,
    "stable_v6_refined_features": STABLE_BENCH,
}
FREEZE_COLOR_VARIANTS = {"f32_gradcache_zero_bg", "f32_accum", "f64_accum64"}
...
def main() -> None:
    p = argparse.ArgumentParser(
        description=(
            "Safe sequential benchmark matrix for the v11_features_gradcache_zero_bg_hostmeta_fixedbin fork. "
    ...
    p.add_argument(
        "--freeze-colors",
        action="store_true",
        help="Only benchmark the f32_gradcache_zero_bg fork path where feature/color tensors do not require gradients.",
    )
    ...
    feature_caps = _csv_ints(args.feature_caps) if args.feature_caps else [max(64, int(args.feature_dim))]
    seeds = _csv_ints(args.seeds)
    cases = _csv_strs(args.cases)
    variant_names = _csv_strs(args.variants)
    if args.include_stable_baseline:
        variant_names.append("stable_v6_refined_features")
    unknown = sorted(set(variant_names) - set(BENCHMARKS))
    if unknown:
        raise ValueError(f"Unknown benchmark variants: {unknown}. Known variants: {sorted(BENCHMARKS)}")
    variants = [(name, BENCHMARKS[name]) for name in dict.fromkeys(variant_names)]

    jobs = list(itertools.product(variants, cases, seeds, batch_sizes, strategies, tile_sizes, chunks, caps, feature_caps))
    ...
    skipped_count = 0
    dry_run_count = 0
    for (variant, bench), case, seed, batch_size, strategy, tile, chunk, cap, feature_cap in jobs:
        env = os.environ.copy()
        env_overrides = {
            "GSP_TILE_SIZE": str(tile),
            "GSP_CHUNK": str(chunk),
            "GSP_FAST_CAP": str(cap),
            "GSP_FEATURE_CAP": str(feature_cap),
        }
        env.update(env_overrides)
        if args.freeze_colors and variant not in FREEZE_COLOR_VARIANTS:
            row = {
                "variant": variant,
                "benchmark": str(bench),
                "command": [],
                ...
                "freeze_colors": True,
                "reason": "--freeze-colors exists only in fork benchmark scripts.",
            }
            _jsonl_write(args.output_jsonl, row)
            skipped_count += 1
            print(
                f"{variant:25s} case={case:18s} status=skipped_unsupported "
                "--freeze-colors is fork-only",
                flush=True,
            )
            continue
        cmd = _build_command(
        ...
        if row["status"] == "ok":
            ...
            dry_run_count += 1
            print(f"{variant:25s} case={case:18s} status=dry_run dry_run", flush=True)
        else:
    ...
    non_ok_count = len(jobs) - ok_count - skipped_count - dry_run_count
    print(
        f"completed {len(jobs)} jobs: ok={ok_count} dry_run={dry_run_count} "
        f"skipped={skipped_count} non_ok={non_ok_count}",
        flush=True,
    )
