...
import torch_gsplat_bridge_v11_features_gradcache_zero_bg_hostmeta_fixedbin.rasterize as v5_rasterize
from torch_gsplat_bridge_v11_features_gradcache_zero_bg_hostmeta_fixedbin import RasterConfig, rasterize_projected_gaussians
...
def run_raster_case(
    ...
    out, _alpha = rasterize_projected_gaussians(means2d_i, conics_i, colors_i, opacities_i, depths.detach().clone(), cfg)
...
def check_case(B: int):
    ...
    cfg = RasterConfig(height=H, width=W, tile_size=16, max_fast_pairs=128)
    out, _alpha = rasterize_projected_gaussians(means2d if B > 1 else means2d[0], conics if B > 1 else conics[0], colors if B > 1 else colors[0], opacities if B > 1 else opacities[0], depths if B > 1 else depths[0], cfg)
...
def check_saturated_many_splats():
    ...
    out, _alpha = rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg)
    out.square().mean().backward()
...
def check_presorted_fast_path():
    ...
    with torch.no_grad():
        base_eval = rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg_default)[0].detach().cpu()
    ...
    try:
        ...
        with torch.no_grad():
            fast_eval = rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg_presorted)[0].detach().cpu()
