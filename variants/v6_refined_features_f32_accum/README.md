...
`v6_refined_features_f32_accum` feature backend independently from the older
...
- Package: `torch_gsplat_bridge_v6_refined_features_f32_accum`
...
- v6_refined_features_f32_accum keeps the f32_reduce backward path but changes
  direct fast forward for `F <= 32` to accumulate per-pixel feature values in
  thread-local storage and write the dense output once. This helps eval-forward
  in the pressure probe but did not beat f32_reduce on fwd+bwd total.
