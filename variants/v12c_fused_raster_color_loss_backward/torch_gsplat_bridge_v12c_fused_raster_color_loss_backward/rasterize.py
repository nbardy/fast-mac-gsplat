...
@dataclass(frozen=True)
class FusedLinearSigmoidMSEBackwardResult:
    features: Tensor
    alpha: Tensor
    grad_means2d: Tensor
    grad_conics: Tensor
    grad_colors: Tensor
    grad_opacities: Tensor
    grad_depths: Tensor
...
def _check_inputs(means2d: Tensor, conics: Tensor, colors: Tensor, opacities: Tensor, depths: Tensor) -> None:
    ...
    if means2d.device.type != "mps":
        raise ValueError("v12c_fused_raster_color_loss_backward Metal rasterizer inputs must be on MPS")
...
def _normalize_bhwc_rgb(name: str, value: Tensor, *, batch_size: int, height: int, width: int, device: torch.device) -> Tensor:
    if value.device != device:
        raise ValueError(f"{name} must be on the same device as raster inputs")
    if value.dtype != torch.float32:
        raise ValueError(f"{name} must be float32")
    if value.ndim == 3:
        if tuple(value.shape) == (height, width, 3):
            value = value.unsqueeze(0)
        elif tuple(value.shape) == (3, height, width):
            value = value.permute(1, 2, 0).unsqueeze(0)
        else:
            raise ValueError(f"{name} must have shape [H,W,3], [3,H,W], [B,H,W,3], or [B,3,H,W]")
    elif value.ndim == 4:
        if tuple(value.shape) == (batch_size, height, width, 3):
            pass
        elif tuple(value.shape) == (batch_size, 3, height, width):
            value = value.permute(0, 2, 3, 1)
        else:
            raise ValueError(f"{name} must have shape [B,H,W,3] or [B,3,H,W] matching raster metadata")
    else:
        raise ValueError(f"{name} must be rank 3 or 4")
    if value.shape[0] == 1 and batch_size != 1:
        value = value.expand(batch_size, -1, -1, -1)
    if tuple(value.shape) != (batch_size, height, width, 3):
        raise ValueError(f"{name} shape must resolve to {(batch_size, height, width, 3)}, got {tuple(value.shape)}")
...
class _RasterizeProjectedGaussiansV6RefinedFeatures(torch.autograd.Function):
    ...
    def forward(
        ...
        if not hasattr(torch.ops, "gsplat_metal_v12c_fused_raster_color_loss_backward"):
            raise RuntimeError("gsplat_metal_v12c_fused_raster_color_loss_backward custom ops not found. Build the extension first.")
        ...
        tile_counts, tile_offsets, binned_ids = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.bin(
        ...
        if use_active_tiles:
            ...
            out_fast, alpha_fast, tile_stop_counts = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_active_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
            ...
            out_fast, alpha_fast, tile_stop_counts = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_fast_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
    ...
    def backward(ctx, grad_features: Tensor | None, grad_alpha: Tensor | None):
        ...
        if ctx.use_active_tiles:
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_active_backward_saved(
                grad_fast,
                grad_alpha_fast,
                means_flat,
                conics_flat,
            ...
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_fast_backward_saved(
                grad_fast,
                grad_alpha_fast,
                means_flat,
                conics_flat,
        ...
        if has_overflow:
            ...
            go_means, go_conics, go_colors, go_opacities = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_overflow_backward(
                grad_tiles,
                grad_alpha_tiles,
                means_flat,
                conics_flat,
...
def _rasterize_chunk_eval(
    ...
    if use_active_tiles:
        ...
        out_fast, alpha_fast = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_active_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
        ...
        out_fast, alpha_fast = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_fast_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
...
def fused_linear_sigmoid_mse_backward(
    means2d: Tensor,
    conics: Tensor,
    colors: Tensor,
    opacities: Tensor,
    ...
    config: RasterConfig,
    *,
    background_rgb: Tensor | None = None,
    loss_scale: float = 1.0,
    compute_color_param_grads: bool = True,
) -> FusedLinearSigmoidMSEBackwardResult:
    """Prototype fused backward for linear 1x1 sigmoid colorize + mean MSE.

    This is intentionally not wired into autograd or trainer dispatch. It runs
    inherited fast-tile forward-state rasterization, then a v12c Metal kernel
    computes per-pixel colorize/compose/MSE gradients locally and immediately
    feeds them into the raster contributor-loop VJP. It returns explicit grads
    for comparison and benchmarking.
    """
    _check_inputs(means2d, conics, colors, opacities, depths)
    if not hasattr(torch.ops, "gsplat_metal_v12c_fused_raster_color_loss_backward"):
        raise RuntimeError("gsplat_metal_v12c_fused_raster_color_loss_backward custom ops not found. Build the extension first.")
    if config.active_policy != "off" or config.use_active_tiles not in (None, False):
        raise ValueError("fused_linear_sigmoid_mse_backward prototype only supports the fast-tile active_policy='off' path")
    if bool(config.enable_overflow_fallback):
        raise ValueError("fused_linear_sigmoid_mse_backward prototype is fixedbin/no-overflow only; set enable_overflow_fallback=False")
    ...
    F = colors_b.shape[-1]
    if F > 32:
        raise ValueError(f"fused_linear_sigmoid_mse_backward prototype requires feature_dim <= 32, got {F}")
    _runtime_validate(config, F)
    if color_weight.device != means2d.device or color_bias.device != means2d.device:
        raise ValueError("color_weight/color_bias must be on the same device as raster inputs")
    if color_weight.dtype != torch.float32 or color_bias.dtype != torch.float32:
        raise ValueError("color_weight/color_bias must be float32")
    if tuple(color_weight.shape) != (3, F):
        raise ValueError(f"color_weight must have shape {(3, F)}, got {tuple(color_weight.shape)}")
    if tuple(color_bias.shape) != (3,):
        raise ValueError(f"color_bias must have shape (3,), got {tuple(color_bias.shape)}")

    target_bhwc = _normalize_bhwc_rgb(
        "target_rgb",
        target_rgb,
        batch_size=B,
    ...
    if background_rgb is None:
        background_bhwc = torch.zeros((B, int(config.height), int(config.width), 3), device=means2d.device, dtype=torch.float32)
    else:
        background_bhwc = _normalize_bhwc_rgb(
            "background_rgb",
            background_rgb,
            batch_size=B,
            height=int(config.height),
    ...
    meta = _make_meta(config, means2d.device, B, G, F)
    perm, means2d_s, conics_s, colors_s, opacities_s = _maybe_sort_inputs_by_depth(
        means2d_b,
        conics_b,
        colors_b,
    ...
    if bool((tile_counts > max_fast_pairs).any().item()):
        raise RuntimeError(
            f"Tile overflow detected with max_fast_pairs={max_fast_pairs}. "
            "The fused prototype only supports no-overflow fast-path rows."
        )

    features_bhwf, alpha_bhw, tile_stop_counts = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_fast_forward_state(
        means_flat,
        conics_flat,
        colors_flat,
        opacities_flat,
    ...
    loss_params = torch.tensor(
        [float(loss_scale), 1.0 if compute_color_param_grads else 0.0],
        device=means2d.device,
        dtype=torch.float32,
    )
    g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat, g_color_weight, g_color_bias = (
        torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_fast_backward_linear_sigmoid_mse(
            features_bhwf,
            alpha_bhw,
            target_bhwc,
            background_bhwc,
            color_weight.contiguous(),
    ...
    if config.inputs_sorted_by_depth:
        g_means_b = g_means_flat.view(B, G, 2)
        g_conics_b = g_conics_flat.view(B, G, 3)
    ...
    if was_batched:
        return FusedLinearSigmoidMSEBackwardResult(
            features=features_bhwf,
            alpha=alpha_bhw,
            grad_means2d=g_means_b,
            grad_conics=g_conics_b,
            grad_colors=g_colors_b,
            grad_opacities=g_opacities_b,
            grad_depths=g_depths_b,
            grad_color_weight=g_color_weight,
            grad_color_bias=g_color_bias,
        )
    return FusedLinearSigmoidMSEBackwardResult(
        features=features_bhwf[0],
        alpha=alpha_bhw[0],
        grad_means2d=g_means_b[0],
        grad_conics=g_conics_b[0],
        grad_colors=g_colors_b[0],
        grad_opacities=g_opacities_b[0],
        grad_depths=g_depths_b[0],
        grad_color_weight=g_color_weight,
        grad_color_bias=g_color_bias,
    )
    ...
    *,
    ...
    for b0 in range(0, B, chunk_b):
        ...
        if run_forward or return_image:
            ...
            if use_active_tiles:
                _, _alpha, stop_counts = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_active_forward_state(
                    m_s,
                    q_s,
                    c_s,
                    o_s,
                ...
                _, _alpha, stop_counts = torch.ops.gsplat_metal_v12c_fused_raster_color_loss_backward.render_fast_forward_state(
                    m_s,
                    q_s,
                    c_s,
                    o_s,
