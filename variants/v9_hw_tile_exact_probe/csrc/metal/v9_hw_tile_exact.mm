...
struct V9ExactPixelState {
  float4 c_t [[raster_order_group(0)]];
  uint stop_count [[raster_order_group(0)]];
  uint flags [[raster_order_group(0)]];
};

struct V9ExactImageblock {
  V9ExactPixelState pixel;
};

struct V9ExactFragOut {
...
fragment V9ExactFragOut v9_exact_update_fs(
  ...
  if ((pixel.flags & 1u) == 0u) {
    ...
    const float alpha = first ? 0.25f : 0.5f;
    const float3 color = first ? float3(1.0f, 0.0f, 0.0f) : float3(0.0f, 1.0f, 0.0f);
...
id<MTLRenderPipelineState> build_render_pipeline(id<MTLDevice> device, std::string& error) {
  ...
  desc.label = @"v9_hw_tile_exact_render_probe";
...
TileExactPipelineBundle build_tile_exact_pipelines(id<MTLDevice> device) {
  ...
  if (@available(macOS 11.0, *)) {
    MTLTileRenderPipelineDescriptor* clear_desc = [[MTLTileRenderPipelineDescriptor alloc] init];
    clear_desc.label = @"v9_hw_tile_exact_clear_tile";
    clear_desc.tileFunction = clear_fn;
    clear_desc.threadgroupSizeMatchesTileSize = YES;
    clear_desc.maxTotalThreadsPerThreadgroup = 1024;
    clear_desc.rasterSampleCount = 1;
    clear_desc.colorAttachments[0].pixelFormat = MTLPixelFormatRGBA32Float;

    NSError* err = nil;
    result.clear_pso =
        [device newRenderPipelineStateWithTileDescriptor:clear_desc options:MTLPipelineOptionNone reflection:nil error:&err];
    if (result.clear_pso == nil) {
      result.error = std::string("clear tile pipeline: ") + nserror_to_string(err);
      return result;
    }

    MTLRenderPipelineDescriptor* update_desc = [[MTLRenderPipelineDescriptor alloc] init];
    update_desc.label = @"v9_hw_tile_exact_update_fragments";
    update_desc.vertexFunction = vs_fn;
    update_desc.fragmentFunction = update_fn;
    update_desc.colorAttachments[0].pixelFormat = MTLPixelFormatRGBA32Float;
    update_desc.colorAttachments[0].blendingEnabled = NO;
    update_desc.colorAttachments[0].writeMask = MTLColorWriteMaskNone;
    ...
    err = nil;
    result.resolve_pso =
        [device newRenderPipelineStateWithTileDescriptor:resolve_desc options:MTLPipelineOptionNone reflection:nil error:&err];
    if (result.resolve_pso == nil) {
      result.error = std::string("resolve tile pipeline: ") + nserror_to_string(err);
      return result;
    }
  ...
  result.error = "tile exact imageblock pipelines require macOS 11.0 or newer";
  return result;
}
...
id<MTLRenderPipelineState> cached_tile_execution_pipeline(id<MTLDevice> device) {
  ...
  if (pso == nil && error.empty()) {
    TilePipelineResult result = build_tile_pipeline_for_function(
        device, "v9_tile_exec_probe", @"v9_hw_tile_exact_tile_execution_probe", 1024);
    pso = result.pso;
    error = result.error;
  }
...
void encode_tile_exact_overlap_probe(
    id<MTLCommandBuffer> command_buffer,
  ...
  if (@available(macOS 11.0, *)) {
    ...
    if (render_encoder == nil) {
      @throw [NSException exceptionWithName:@"V9HWTileExact"
                                     reason:@"failed to create tile exact render command encoder"
                                   userInfo:nil];
    }
...
py::dict run_tile_exact_execution_probe_native(
    int64_t height,
    int64_t width,
    int64_t tile_size) {
  TORCH_CHECK(height > 0 && width > 0, "height and width must be positive");
...
py::dict run_tile_exact_overlap_probe_native(
    int64_t height,
    int64_t width,
    int64_t tile_size) {
  TORCH_CHECK(height > 0 && width > 0, "height and width must be positive");
  ...
  TORCH_CHECK(tile_size == 16,
              "tile_size must be 16 for this exact-overlap probe. 32x32 exact state is intentionally disabled "
  ...
  const NSUInteger bytes_per_row = (NSUInteger)width * bytes_per_pixel;
  TORCH_CHECK(bytes_per_row % 256 == 0,
              "tile exact overlap probe uses a direct buffer-backed RGBA32F render target and requires "
              "width*16 to be 256-byte aligned; got bytes_per_row=",
              bytes_per_row);
  ...
  if (compile_advanced) {
    ...
    for (const TileLayoutSpec& spec : kTileLayoutSpecs) {
      TilePipelineResult result = build_tile_pipeline_for_function(
          device,
          spec.function_name,
          [NSString stringWithFormat:@"v9_hw_tile_exact_%s", spec.function_name],
          256);
    ...
    TilePipelineResult exec_result = build_tile_pipeline_for_function(
        device, "v9_tile_exec_probe", @"v9_hw_tile_exact_tile_execution_probe", 256);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def(
      ...
      &run_tile_exact_execution_probe_native,
      py::arg("height") = 32,
      py::arg("width") = 32,
      py::arg("tile_size") = 32,
  ...
  m.def(
      ...
      &run_tile_exact_overlap_probe_native,
      py::arg("height") = 32,
      py::arg("width") = 32,
      py::arg("tile_size") = 32,
