...
def main() -> None:
    ...
    assert any(row["format"] == "r32f" and row["buffer_backed_texture_created"] for row in format_rows)
    assert any(row["format"] == "rg32f" and row["buffer_backed_texture_created"] for row in format_rows)
    ...
    assert not checked.icb_execute_validated, checked.as_dict()
    print(f"icb_execute_validated={checked.icb_execute_validated} (disabled: known AGX crash risk)")
