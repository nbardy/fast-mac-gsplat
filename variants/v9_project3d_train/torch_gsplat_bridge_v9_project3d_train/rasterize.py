...
class _ProjectPinholeGaussiansV9Project3DTrain(torch.autograd.Function):
    ...
    def backward(
        ...
        flat_grads = torch.ops.gsplat_metal_v9_project3d_train.project_pinhole_backward(
            ...
            grad_outputs[3].reshape(-1).contiguous(),
            means3d_b.reshape(-1, 3).contiguous(),
            scales_b.reshape(-1, 3).contiguous(),
            quats_b.reshape(-1, 4).contiguous(),
            opacities_b.reshape(-1).contiguous(),
            ...
            torch.tensor(
                [
                    ...
                    0,
                    0,
                    int(means3d_b.shape[0] * means3d_b.shape[1]),
                    0,
                    0,
                    int(means3d_b.shape[0]),
                    int(means3d_b.shape[1]),
                    0,
                    0,
                ],
        ...
        full_grads: list[Tensor | None] = [
            flat_grads[0].view_as(means3d_b),
            flat_grads[1].view_as(scales_b),
            flat_grads[2].view_as(quats_b),
            flat_grads[3].view_as(opacities_b),
            flat_grads[4],
            flat_grads[5],
        ]
        for index, need in enumerate(ctx.needs_input_grad[:6]):
            if not need:
                full_grads[index] = None
