...
#endif
  TORCH_CHECK(false, "gsplat_metal_v9_project3d_train.bin: no backend available for device ", means2d.device());
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> project_pinhole_forward_dispatch(
...
#if defined(__APPLE__)
  if (means3d.device().is_mps()) {
    return metal_project_pinhole_forward(
        means3d, scales, quats, opacities, camera_to_world, camera_params, meta_i32, project_f32);
  }
#endif
  TORCH_CHECK(false, "gsplat_metal_v9_project3d_train.project_pinhole_forward: no backend available for device ", means3d.device());
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
project_pinhole_backward_dispatch(
...
#if defined(__APPLE__)
  if (means3d.device().is_mps()) {
    return metal_project_pinhole_backward(
        grad_means2d,
        grad_conics,
        grad_projected_opacities,
        grad_depths,
        means3d,
        scales,
        quats,
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v9_project3d_train.project_pinhole_backward: no backend available for device ", means3d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v9_project3d_train.render_fast_forward_eval: no backend available for device ", means2d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v9_project3d_train.render_fast_forward_state: no backend available for device ", means2d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v9_project3d_train.render_fast_backward_saved: no backend available for device ", means2d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v9_project3d_train.render_overflow_forward: no backend available for device ", means2d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v9_project3d_train.render_overflow_backward: no backend available for device ", means2d.device());
}

}  // namespace
}  // namespace gsplat

TORCH_LIBRARY(gsplat_metal_v9_project3d_train, m) {
  ...
  m.def("project_pinhole_forward(Tensor means3d, Tensor scales, Tensor quats, Tensor opacities, Tensor camera_to_world, Tensor camera_params, Tensor meta_i32, Tensor project_f32) -> (Tensor, Tensor, Tensor, Tensor)");
  m.def("project_pinhole_backward(Tensor grad_means2d, Tensor grad_conics, Tensor grad_projected_opacities, Tensor grad_depths, Tensor means3d, Tensor scales, Tensor quats, Tensor opacities, Tensor camera_to_world, Tensor camera_params, Tensor meta_i32, Tensor project_f32) -> (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor)");
...
TORCH_LIBRARY_IMPL(gsplat_metal_v9_project3d_train, CompositeExplicitAutograd, m) {
  m.impl("bin", gsplat::bin_dispatch);
  m.impl("project_pinhole_forward", gsplat::project_pinhole_forward_dispatch);
  m.impl("project_pinhole_backward", gsplat::project_pinhole_backward_dispatch);
