...
def run_smoke(
    *,
    ...
    for step in range(steps):
        ...
        rgb, _alpha, _depth = shared_realray_rgba_depth_autograd(
            boundary_f32,
            candidate_mask_i32,
            sites_f32,
            site_rgba,
        ...
        psnr_history.append(_psnr_from_mse(float(rgb_mse.detach().cpu().item())))
        loss.backward()
        if step == 0:
    ...
    with torch.no_grad():
        ...
        target_rgb_cpu = targets.detach().cpu()
        final_rgb_mse = float(F.mse_loss(final_rgb_image, target_rgb_cpu).item())

    initial_rgb_mse = loss_history[0]
    initial_train_psnr = _psnr_from_mse(initial_rgb_mse)
    final_train_psnr = _psnr_from_mse(final_rgb_mse)
    loss_ratio = final_rgb_mse / float(max(initial_rgb_mse, 1.0e-12))
    ...
    acceptance = {
        ...
        "optimized_against_real_rgb_targets": True,
        "loss_decreased": final_rgb_mse < initial_rgb_mse,
        "train_psnr_improved": final_train_psnr > initial_train_psnr,
        "parameters_updated": parameter_update_abs_max > 1.0e-6,
        "gradients_nonzero": first_grad_abs_sum > 0.0 and first_grad_abs_max > 0.0,
        "all_losses_finite": all(torch.isfinite(torch.tensor(value)).item() for value in [*loss_history, final_rgb_mse]),
    ...
    return {
        "benchmark": "world_foam_lane2_gate3_mps_shared_realray_real_target_train_smoke",
        "status": "ok" if all(acceptance.values()) else "failed",
        "gate": "3_realray_mps_frozen_geometry_site_rgba_real_target_train",
        "device": "mps",
        ...
        "quality_claim": False,
        "real_target_training_smoke_claim": True,
        "full_trainer_claim": False,
        "parameter_update_claim": True,
        "world_foam_renderer_status": "mps_shared_real_camera_ray_frozen_geometry_site_rgba_real_target_train_smoke_no_geometry_topology_gradients_no_quality_claim",
        "steps": steps,
        "lr": lr,
        "initial_rgb_mse": initial_rgb_mse,
        "final_rgb_mse": final_rgb_mse,
        "loss_ratio": loss_ratio,
        "initial_train_psnr": initial_train_psnr,
        "final_train_psnr": final_train_psnr,
        "train_psnr_delta": final_train_psnr - initial_train_psnr,
        ...
        "train": {
            ...
            "target_rgb_shape": list(target_rgb_cpu.shape),
            ...
            "max_final_rgb_abs_error_to_target": float((final_rgb_image - target_rgb_cpu).abs().max().item()),
            "final_alpha_min": float(final_alpha.detach().cpu().min().item()),
            "final_alpha_max": float(final_alpha.detach().cpu().max().item()),
            "final_depth_min": float(final_depth.detach().cpu().min().item()),
            "final_depth_max": float(final_depth.detach().cpu().max().item()),
        },
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke World Foam shared true real-camera-ray MPS frozen-geometry real-target training."
    )
