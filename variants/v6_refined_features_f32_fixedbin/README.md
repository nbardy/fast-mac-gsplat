...
`variants/v6_refined_features_f32_gradcache`, created to test fixed-cap
per-tile bin storage without mutating the stable `v6_refined_features` baseline
or the earlier optimization forks.
...
- Package: `torch_gsplat_bridge_v6_refined_features_f32_fixedbin`
...
- this fork is intentionally no-overflow only; Python raises if any tile count
  exceeds `max_fast_pairs`
- `auto` batch mode chunks large batches to cap total launched tiles / gaussians
- `v6_refined_features_f32_fixedbin` removes the exact-length
  `cumsum -> cat -> clone -> final_offset.item() -> empty(N)` bin allocation
  path and instead allocates a fixed `tile_count * max_fast_pairs` int32 ID
  buffer. This trades a larger fixed buffer for avoiding the bin-size sync and
  allocation path.
- At `512px/B16/tile16/cap2048`, the fixed ID buffer is about `128 MiB`.
- It inherits the `f32_gradcache` direct fast-backward path, passed feature and
  alpha correctness gates, and passed exact trainer fixed-render forward plus
  gradient parity against stable `v6_refined_features`.
- Timing is mixed: it wins the target synthetic `512px/B16/G8192/F32` row,
  slightly wins `256px/B16/G8192/F64`, loses `256px/B16/G8192/F32`, and ties
  `f32_accum` on the latest 256px trainer fixed-render graph.
...
- Keep original `v6_refined_features` as the stable v6-refined feature baseline;
  use this fork only when callers explicitly want the fixed-bin no-overflow
  experiment.
