...
inline ParsedMeta parse_meta(const torch::Tensor& meta_i32, const torch::Tensor& meta_f32) {
  TORCH_CHECK(meta_i32.device().is_cpu(), "host meta_i32 must be on CPU");
  TORCH_CHECK(meta_f32.device().is_cpu(), "host meta_f32 must be on CPU");
  auto* ip = meta_i32.data_ptr<int32_t>();
  auto* fp = meta_f32.data_ptr<float>();
  ParsedMeta out;
  out.height = ip[0];
  out.width = ip[1];
  out.tiles_y = ip[2];
