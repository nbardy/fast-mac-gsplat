...
_HARDWARE_EVAL_REASON = (
    "hardware tile/imageblock eval is not selected: "
    "there is no MPS-visible render target interop, GPU-generated draw stream, "
    "or exact final_T/stop_count capture yet; using the inherited v8 compute path"
)


@dataclass(frozen=True)
class HardwareEvalCapabilities:
    mps_available: bool
    native_ops_registered: bool
    native_probe_available: bool
    native_probe_error: str
    metal_available: bool
    metal_device_name: str
    command_queue_created: bool
    render_pipeline_source_available: bool
    render_pipeline_compile_attempted: bool
    render_pipeline_library_compiled: Optional[bool]
    render_pipeline_ready: Optional[bool]
    render_pipeline_error: str
    no_cpu_readback: bool
    cpu_readback_path_present: bool
    render_pipeline_uses_cpu_readback: bool
    render_to_mps_interop: bool
    render_to_mps_interop_reason: str
    imageblock_support_known: bool
    imageblock_supported: Optional[bool]
    imageblock_support_reason: str
    raster_order_group_support_known: bool
    raster_order_group_supported: Optional[bool]
    raster_order_group_support_reason: str
...
@dataclass(frozen=True)
class HardwareEvalStatus:
    requested: bool
    supported: bool
    used_hardware_eval: bool
    fallback_to_v8_compute: bool
    policy: str
    reason: str
    native_ops_registered: bool
    mps_available: bool
    emit_final_T: bool
    emit_stop_count: bool
    selection: str
    missing_prerequisites: Tuple[str, ...]
    unknown_prerequisites: Tuple[str, ...]
    capabilities: HardwareEvalCapabilities
...
class RasterConfig:
    ...
    use_hardware_eval: bool = False
    hardware_eval_policy: str = "fallback"  # off | fallback | require
    emit_final_T: bool = False
    emit_stop_count: bool = False
...
def _runtime_validate(config: RasterConfig) -> RuntimeShaderConfig:
    ...
    if config.hardware_eval_policy not in ("off", "fallback", "require"):
        raise ValueError("hardware_eval_policy must be one of: off, fallback, require")
...
def _native_ops_registered() -> bool:
    try:
        getattr(torch.ops.gsplat_metal_v8_hw_eval, "bin")
...
@lru_cache(maxsize=2)
def _native_hardware_eval_probe(compile_render_pipeline: bool) -> Dict[str, Any]:
    if _C is None:
        return {
            "native_probe_available": False,
            "native_probe_error": "native extension is not imported; build variants/v8_hw_eval first",
        }
    probe = getattr(_C, "probe_hardware_eval_native", None)
    if probe is None:
        return {
            "native_probe_available": False,
            "native_probe_error": "native extension does not expose probe_hardware_eval_native",
        }
    try:
        return dict(probe(bool(compile_render_pipeline)))
...
def probe_hardware_eval_capabilities(*, compile_render_pipeline: bool = True) -> HardwareEvalCapabilities:
    native = _native_hardware_eval_probe(bool(compile_render_pipeline))
    return HardwareEvalCapabilities(
        mps_available=bool(torch.backends.mps.is_available()),
        native_ops_registered=_native_ops_registered(),
        ...
        command_queue_created=bool(native.get("command_queue_created", False)),
        render_pipeline_source_available=bool(native.get("render_pipeline_source_available", False)),
        render_pipeline_compile_attempted=bool(native.get("render_pipeline_compile_attempted", False)),
        render_pipeline_library_compiled=_optional_bool(native.get("render_pipeline_library_compiled")),
        ...
        no_cpu_readback=bool(native.get("no_cpu_readback", True)),
        cpu_readback_path_present=bool(native.get("cpu_readback_path_present", False)),
        render_pipeline_uses_cpu_readback=bool(native.get("render_pipeline_uses_cpu_readback", False)),
        render_to_mps_interop=bool(native.get("render_to_mps_interop", False)),
        render_to_mps_interop_reason=str(native.get("render_to_mps_interop_reason", "not implemented")),
        imageblock_support_known=bool(native.get("imageblock_support_known", False)),
        imageblock_supported=_optional_bool(native.get("imageblock_supported")),
        imageblock_support_reason=str(native.get("imageblock_support_reason", "not probed")),
        raster_order_group_support_known=bool(native.get("raster_order_group_support_known", False)),
        raster_order_group_supported=_optional_bool(native.get("raster_order_group_supported")),
        raster_order_group_support_reason=str(native.get("raster_order_group_support_reason", "not probed")),
        native_details=native,
    )


def _hardware_eval_prerequisites(
    capabilities: HardwareEvalCapabilities,
    *,
    require_render_pipeline_probe: bool,
) -> tuple[Tuple[str, ...], Tuple[str, ...]]:
    missing: list[str] = []
    unknown: list[str] = []
    if not capabilities.mps_available:
        missing.append("mps")
    if not capabilities.native_ops_registered:
        missing.append("native_ops")
    if not capabilities.native_probe_available:
        missing.append("native_probe")
    if not capabilities.metal_available:
        missing.append("metal_device")
    if not capabilities.command_queue_created:
        missing.append("metal_command_queue")
    if not capabilities.render_pipeline_source_available:
        missing.append("render_pipeline_source")
    if require_render_pipeline_probe:
        if capabilities.render_pipeline_ready is not True:
            missing.append("render_pipeline_probe")
    elif capabilities.render_pipeline_ready is None:
        unknown.append("render_pipeline_probe")
    elif capabilities.render_pipeline_ready is False:
        missing.append("render_pipeline_probe")
    if (
        not capabilities.no_cpu_readback
        or capabilities.cpu_readback_path_present
        or capabilities.render_pipeline_uses_cpu_readback
    ):
        missing.append("no_cpu_readback")
    if not capabilities.render_to_mps_interop:
        missing.append("render_to_mps_interop")
    if capabilities.imageblock_support_known:
        if capabilities.imageblock_supported is not True:
            missing.append("imageblock_support")
    else:
        unknown.append("imageblock_support")
    if capabilities.raster_order_group_support_known:
        if capabilities.raster_order_group_supported is not True:
            missing.append("raster_order_group_support")
    else:
        unknown.append("raster_order_group_support")
    return tuple(missing), tuple(unknown)


def _hardware_eval_reason(
    *,
    policy: str,
    requested: bool,
    supported: bool,
    ...
    if policy == "off":
        return "hardware eval disabled by policy; using the inherited v8 compute path"
    if not requested:
        return "hardware eval not requested; using the inherited v8 compute path"
    if supported:
        return "hardware eval prerequisites passed"
    details = []
    if missing:
    ...
    if not details:
        return _HARDWARE_EVAL_REASON
    return "hardware eval not selected: " + "; ".join(details) + "; using the inherited v8 compute path"


def probe_hardware_eval(
    config: RasterConfig | None = None,
    *,
    compile_render_pipeline: bool = True,
) -> HardwareEvalStatus:
    policy = config.hardware_eval_policy if config is not None else "fallback"
    requested = bool(config.use_hardware_eval) if config is not None and policy != "off" else False
    emit_final_T = bool(config.emit_final_T) if config is not None else False
    emit_stop_count = bool(config.emit_stop_count) if config is not None else False
    capabilities = probe_hardware_eval_capabilities(compile_render_pipeline=compile_render_pipeline)
    missing, unknown = _hardware_eval_prerequisites(
        capabilities,
        require_render_pipeline_probe=bool(requested or compile_render_pipeline),
    )
    supported = bool(not missing and not unknown)
    used = bool(requested and supported)
    selection = "hardware_eval" if used else "v8_compute_fallback"
    return HardwareEvalStatus(
        requested=requested,
        supported=supported,
        used_hardware_eval=used,
        fallback_to_v8_compute=not used,
        policy=policy,
        reason=_hardware_eval_reason(
            policy=policy,
            requested=requested,
            supported=supported,
            missing=missing,
        ...
        native_ops_registered=capabilities.native_ops_registered,
        mps_available=capabilities.mps_available,
        emit_final_T=emit_final_T,
        emit_stop_count=emit_stop_count,
        selection=selection,
...
def _enforce_hardware_eval_policy(config: RasterConfig, *, compile_render_pipeline: bool = False) -> HardwareEvalStatus:
    status = probe_hardware_eval(config, compile_render_pipeline=compile_render_pipeline or bool(config.use_hardware_eval))
    if config.use_hardware_eval and config.hardware_eval_policy == "require" and not status.supported:
...
def _choose_batch_slices(
    ...
    *,
    ...
    for b0, b1 in base_slices:
        ...
        tile_counts, _, _ = torch.ops.gsplat_metal_v8_hw_eval.bin(
...
class _RasterizeProjectedGaussiansV8(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means2d_b: Tensor,
        ...
        if not hasattr(torch.ops, "gsplat_metal_v8_hw_eval"):
            raise RuntimeError("gsplat_metal_v8_hw_eval custom ops not found. Build the extension first.")
        ...
        tile_counts, tile_offsets, binned_ids = torch.ops.gsplat_metal_v8_hw_eval.bin(
        ...
        if resolved_use_active_tiles:
            ...
            out_fast, tile_stop_counts = torch.ops.gsplat_metal_v8_hw_eval.render_active_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
            ...
            out_fast, tile_stop_counts = torch.ops.gsplat_metal_v8_hw_eval.render_fast_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
        ...
        if enable_overflow_fallback:
            ...
            if overflow_tile_ids.numel() > 0:
                overflow_tile_imgs = torch.ops.gsplat_metal_v8_hw_eval.render_overflow_forward(
                    means_flat,
                    conics_flat,
                    colors_flat,
                    opacities_flat,
    ...
    def backward(ctx, grad_out: Tensor):
        ...
        if ctx.use_active_tiles:
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v8_hw_eval.render_active_backward_saved(
                grad_fast,
                means_flat,
                conics_flat,
                colors_flat,
            ...
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v8_hw_eval.render_fast_backward_saved(
                grad_fast,
                means_flat,
                conics_flat,
                colors_flat,
        ...
        if ctx.enable_overflow_fallback and overflow_tile_ids.numel() > 0:
            ...
            go_means, go_conics, go_colors, go_opacities = torch.ops.gsplat_metal_v8_hw_eval.render_overflow_backward(
                grad_tiles,
                means_flat,
                conics_flat,
                colors_flat,
...
def _rasterize_chunk_eval(
    ...
    if resolved_use_active_tiles:
        ...
        out_fast = torch.ops.gsplat_metal_v8_hw_eval.render_active_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
        ...
        out_fast = torch.ops.gsplat_metal_v8_hw_eval.render_fast_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
...
def _rasterize_batched(
    ...
    _enforce_hardware_eval_policy(config)
    B, G = means2d_b.shape[:2]
    tiles_y = (config.height + config.tile_size - 1) // config.tile_size
    tiles_x = (config.width + config.tile_size - 1) // config.tile_size
    ...
    for b0, b1 in batch_slices:
        ...
        if train_mode:
            meta = _make_meta(config, m.device, b1 - b0, G)
            outs.append(
                _RasterizeProjectedGaussiansV8.apply(
                    m,
                    q,
                    c,
                    o,
...
def profile_projected_gaussians(
    ...
    *,
    ...
    B, G = means2d_b.shape[:2]
    _runtime_validate(config)
    hardware_status = _enforce_hardware_eval_policy(config, compile_render_pipeline=True)
    ...
    for b0, b1 in batch_slices:
        ...
        if run_forward or return_image:
            ...
            if resolved_use_active_tiles:
                _, stop_counts = torch.ops.gsplat_metal_v8_hw_eval.render_active_forward_state(
                    m_s,
                    q_s,
                    c_s,
                    o_s,
                ...
                _, stop_counts = torch.ops.gsplat_metal_v8_hw_eval.render_fast_forward_state(
                    m_s,
                    q_s,
                    c_s,
                    o_s,
