...
import torch_gsplat_bridge_v6_refined_features as v6_refined_features
...
def check_shapes() -> None:
    ...
    for feature_dim in (1, 3, 4, 8, 16, 32, 64):
        ...
        with torch.no_grad():
            ...
            out_s, alpha_s = v6_refined_features.rasterize_projected_gaussians(means2d[0], conics[0], colors[0], opacities[0], depths[0], cfg)
...
def check_no_nan_smoke() -> None:
    ...
    for i in range(100):
        ...
        out, _alpha = v6_refined_features.rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg)
        loss = out.square().mean()
        loss.backward()
