...
def run_probe(args: argparse.Namespace) -> dict[str, Any]:
    if not torch.backends.mps.is_available():
        raise RuntimeError("MPS is required for the atlas render timing probe")
    ...
    torch.mps.synchronize()
    parity = _image_metrics(cached_image.detach().cpu(), scan_image.detach().cpu())
    ...
    speedup = scan["median_ms"] / max(cached["median_ms"], 1.0e-9)
    return {
        "name": "depth_banded_homography_flow_atlas_metal_render_timing_probe",
        "note": (
            "F1c timing probe for inverse-homography atlas-residual render kernels. "
            "The cached variant stores per-pixel candidate ids/depths before compositing; "
            "it is still a correctness-preserving intermediate, not the final optimized renderer."
        ),
        "config": {
            "seed": args.seed,
        ...
        "cached_vs_scan": parity,
        "scan_render": scan,
        "cached_render": cached,
        "cached_speedup_vs_scan": speedup,
        "pass": bool(parity["max_abs"] <= args.max_abs_gate and speedup >= args.min_speedup_gate),
    }
...
def main() -> None:
    ...
    parser.add_argument("--min-speedup-gate", type=float, default=1.05)
