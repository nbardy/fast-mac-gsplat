...
- [x] Gate F1f: 256-tube scaling boundary rejects cached atlas cap32 without candidate scaling.
- [x] Gate F1g: cap64 cached atlas restores 256-tube parity but loses valid PRT tiled speed.
- [x] Gate F1h: cap64 cached atlas still beats scan at 256 tubes but not valid PRT tiled.
- [x] Gate F1i: cached-select ordering is exact but slower than insertion-cached.
- [x] Gate F1j: support 1.25 recovers a speed-mode win at 256 tubes with non-exact dense coverage.
- [x] Gate F1k: 128px stress keeps support 1.25 speed win and support 1.4 fidelity split.
- [x] Gate F1l: overfit-artifact audit separates PRT training evidence from atlas support-policy evidence.
...
- [x] Gate F1r: CPU/direct atlas-residual overfit gate proves differentiable training quality on a tiny moving-camera target.
- [x] Gate F1s: bounded SE3 learnable-camera PRT gate proves camera-pose params receive Metal PRT gradients.
- [x] Gate F1t: real-clip PRT world-tube trainer with bounded learnable train-camera residuals and video artifact logging.
- [x] Gate F1u: stronger real-clip W&B row logs per-step loss curves and improves the world-tube overfit fit.
...
Gate F0j checks that the F0i `1.5x` atlas support is not a one-seed accident.
It also vectorizes the dense active-candidate audit inside the CPU tiled
reference, reducing the support-1.5 seed-17 smoke from an opaque minute-scale
run to a bounded short check with the same numbers.

Command:

```text
python3 research_project/benchmarks/depth_banded_homography_flow_atlas_tiled_seed_sweep.py --out-json research_project/benchmarks/results/depth_banded_homography_flow_atlas_tiled_seed_sweep_f0j_64_16f_128t_4seeds_tile4_support15.json
```

Result:

```text
Seeds 17, 23, 31, 47: pass true for all rows.
Missing active candidates: min 0, median 0, max 0.
PSNR vs dense atlas-residual render: min 120.0 dB, median 120.0 dB, max 120.0 dB.
Fallback tubes: min 0, median 0, max 0.
Candidate eval ratio vs dense: min 0.0743, median 0.0780, max 0.0812.
Tile pairs: min 9646, median 10041, max 10362.
Max candidates per pixel: min 75, median 80, max 83.
```

Read: `support_scale=1.5` is the current CPU reference contract for the first
Metal atlas-tiled path. It exactly covers dense active candidates across the
...
Gate F1a moves the first half of the inverse-homography atlas-residual renderer
onto Metal without claiming the full renderer yet. It adds a bin-only op,
`bin_inverse_homography_atlas_residual_tiles`, whose tile id is exactly the CPU
atlas convention:

```text
(((band * tiles_t + tz) * tiles_y + ty) * tiles_x) + tx
```

The op takes `atlas_ref_uv`, `atlas_residual_coeff`, `lambda_uv`, `lambda_t`,
`center_t`, `opacity`, and `band_ids`; it returns `tile_counts`,
`tile_overflow`, and `tile_tube_ids`. The benchmark compares per-tile tube-id
sets, not raw slot order, because Metal atomic insertion order is not stable.

Commands:

```text
PYTHONPATH=variants/star_uvt_prt_v0 STAR_UVT_TILE_X=4 STAR_UVT_TILE_Y=4 STAR_UVT_TILE_T=4 STAR_UVT_TILE_CAPACITY=512 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_metal_binning_probe.py --seed 17 --target-size 64 --frames 16 --tubes 128 --depth-bands 4 --residual-degree 3 --prt-degree 2 --tile-size 4 --tile-t 4 --support-scale 1.4 --tile-capacity 512 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_metal_binning_probe_f1a_64_16f_128t_seed17_tile4_support14.json
PYTHONPATH=variants/star_uvt_prt_v0 STAR_UVT_TILE_X=4 STAR_UVT_TILE_Y=4 STAR_UVT_TILE_T=4 STAR_UVT_TILE_CAPACITY=32 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_metal_binning_probe.py --seed <17|23|31|47> --target-size 64 --frames 16 --tubes 128 --depth-bands 4 --residual-degree 3 --prt-degree 2 --tile-size 4 --tile-t 4 --support-scale 1.4 --tile-capacity 32 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_metal_binning_probe_f1a_64_16f_128t_seed<seed>_tile4_support14_cap32.json
```

Result:

```text
Four-seed cap32 sweep: pass true for seeds 17, 23, 31, 47.
Per-tile count mismatches: 0 on every row.
Per-tile set mismatches: 0 on every row.
Missing pairs / extra pairs: 0 / 0 on every row.
Duplicate or out-of-range Metal ids: 0 on every row.
Overflow tiles: 0 on every row.
Tile pairs: min 8747, median 9028, max 9338.
Active tiles: min 1508, median 1540, max 1599.
Max tile count: min 25, median 26, max 29.
```

Read: the atlas tile assignment is now a real Metal path and matches the CPU
reference exactly at the F0k target scale. This is still not the full
sublinear renderer: the next slice must inverse-warp screen pixels per depth
band, gather the banded atlas tile candidates, sort/compose them, and compare
the rendered image against the dense F0h/F0i reference.

Gate F1b adds that first full render parity slice. It is deliberately
correctness-first rather than fast: each output pixel inverse-warps into each
depth-band atlas, scans the selected atlas tile lists, repeatedly selects the
next candidate in depth order, and composites in screen space. This proves the
complete Metal path from atlas tile bins to pixels, but it is not the final
optimized renderer.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 STAR_UVT_TILE_X=4 STAR_UVT_TILE_Y=4 STAR_UVT_TILE_T=4 STAR_UVT_TILE_CAPACITY=32 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_metal_render_probe.py --seed <17|23|31|47> --target-size 64 --frames 16 --tubes 128 --depth-bands 4 --residual-degree 3 --prt-degree 2 --tile-size 4 --tile-t 4 --support-scale 1.4 --tile-capacity 32 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_metal_render_probe_f1b_64_16f_128t_seed<seed>_tile4_support14_cap32.json
```

Result:

```text
Four-seed cap32 render sweep: pass true for seeds 17, 23, 31, 47.
Metal vs CPU tiled PSNR: 120 dB on every row.
Metal vs dense PSNR: 120 dB on every row.
Metal vs dense max_abs: min 3.87e-07, median 7.60e-07, max 8.94e-07.
Overflow tiles: 0 on every row.
Tile pairs: min 8747, median 9073, max 9338.
Candidate eval ratio in the CPU reference: min 0.0675, median 0.0707, max 0.0732.
```

Read: the inverse-homography atlas-residual path now has a full Metal
correctness path at the F0k target scale. The remaining work is speed:
replace the per-pixel repeated scan with an efficient gather/sort strategy,
...
Gate F1d runs that first same-scene comparison. It keeps the cached-compatible
configuration (`depth_bands=4`, `tile_capacity=32`, candidate capacity 128) and
compares four render paths on the same generated world-tube scene: F1b scan
atlas, F1c cached atlas, dense direct per-frame world-tube render, PRT direct
Metal render, and PRT tiled Metal render under the same cap32 process-static
tile config.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 STAR_UVT_TILE_X=4 STAR_UVT_TILE_Y=4 STAR_UVT_TILE_T=4 STAR_UVT_TILE_CAPACITY=32 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_render_compare_probe.py --seed <17|23|31|47> --target-size 64 --frames 16 --tubes 128 --depth-bands 4 --residual-degree 3 --prt-degree 2 --tile-size 4 --tile-t 4 --support-scale 1.4 --tile-capacity 32 --warmup 2 --iters 5 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_render_compare_f1d_64_16f_128t_seed<seed>_tile4_t4_support14_cap32_cached_scan_direct.json
```

Result:

```text
seed 17: cached 3.972 ms, scan 5.813 ms, dense direct 218.730 ms, PRT direct 9.008 ms, PRT tiled 2.083 ms invalid; cached speedups 1.464x vs scan, 55.075x vs dense direct, 2.268x vs PRT direct
seed 23: cached 5.225 ms, scan 7.776 ms, dense direct 319.128 ms, PRT direct 8.051 ms, PRT tiled 2.080 ms invalid; cached speedups 1.488x vs scan, 61.080x vs dense direct, 1.541x vs PRT direct
seed 31: cached 4.857 ms, scan 6.798 ms, dense direct 199.355 ms, PRT direct 11.640 ms, PRT tiled 2.856 ms invalid; cached speedups 1.400x vs scan, 41.042x vs dense direct, 2.396x vs PRT direct
seed 47: cached 6.099 ms, scan 9.050 ms, dense direct 201.364 ms, PRT direct 8.735 ms, PRT tiled 2.417 ms invalid; cached speedups 1.484x vs scan, 33.015x vs dense direct, 1.432x vs PRT direct
```

Validity:

```text
Cached atlas path: pass true on all four seeds; cached-vs-scan max_abs 0; direct-dense PSNR 92.68/96.20/94.09/97.57 dB; atlas overflow 0.
PRT tiled under cap32: invalid on all four seeds; overflow tiles 68/80/68/77 with max tile counts 74/84/72/78.
```

Read: the cached atlas renderer is now a useful forward-render speed row, not
just a scan-kernel micro-optimization. It beats the dense direct per-frame
world-tube reference and the existing PRT direct Metal renderer on this exact
128-tube moving-camera scene. It does not yet beat a valid PRT tiled baseline:
the only same-process cap32 PRT tiled row is faster but drops candidates from
overflow, so it is not a correctness-preserving comparison. The next fork
iteration should either raise/cache candidates beyond 128, split by band/window,
or run a two-process comparison that measures cached atlas at cap32 against a
...
Gate F1f runs the first scale boundary row at 256 tubes. The coordinator now
preserves child JSON even when a child reports `pass=false`, so the negative
atlas result is saved instead of discarded. The PRT child also treats dense
direct per-frame render as a valid reference when PRT direct is the outlier; on
seed 23, PRT tiled matched dense direct to `1.43e-06` max error while PRT direct
had a single `2.34e-04` discrepancy.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_probe.py --seed <17|23|31|47> --target-size 64 --frames 16 --tubes 256 --depth-bands 4 --residual-degree 3 --prt-degree 2 --atlas-tile-config 4x4x4:32 --prt-tile-config 4x4x4:256 --support-scale 1.4 --warmup 2 --iters 5 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_f1f_64_16f_256t_seed<seed>_atlas4x4x4cap32_prt4x4x4cap256.json
```

Result:

```text
seed 17: atlas pass false, max tile count 52, overflow tiles 83, cached-vs-scan max_abs 0.2206; PRT tiled pass true, max tile count 146, overflow 0
seed 23: atlas pass false, max tile count 53, overflow tiles 81, cached-vs-scan max_abs 0.0722; PRT tiled pass true, max tile count 150, overflow 0
seed 31: atlas pass false, max tile count 48, overflow tiles 66, cached-vs-scan max_abs 0.1069; PRT tiled pass true, max tile count 147, overflow 0
seed 47: atlas pass false, max tile count 55, overflow tiles 105, cached-vs-scan max_abs 0.1232; PRT tiled pass true, max tile count 150, overflow 0
```

Read: the current cached atlas implementation is bounded by atlas tile capacity,
not by the PRT tiled baseline. `4x4x4:32` is enough for the 128-tube F1e row but
fails every 256-tube seed. The speed numbers from failed atlas rows are not
promotion evidence because candidates were dropped. The next implementation
step should be explicit candidate scaling for cached atlas: streamed candidate
blocks, per-band/window split passes, or a larger local candidate store. Until
one of those exists, do not wire this path into training/backward or claim
large-scene playback.

Gate F1g tests the smallest candidate-scaling change: raise the cached atlas
per-pixel local candidate array and wrapper/host guards from `128` to `256`, then
run the 256-tube row with cached atlas at `4x4x4:64`. This is not a new
algorithm; it is a capacity bump to test whether correctness and speed move in
the right direction.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_probe.py --seed <17|23|31|47> --target-size 64 --frames 16 --tubes 256 --depth-bands 4 --residual-degree 3 --prt-degree 2 --atlas-tile-config 4x4x4:64 --prt-tile-config 4x4x4:256 --support-scale 1.4 --warmup 2 --iters 5 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_f1g_64_16f_256t_seed<seed>_atlas4x4x4cap64_prt4x4x4cap256.json
```

Result:

```text
seed 17: atlas pass true, max tile count 52, overflow 0, cached 10.894 ms, valid PRT tiled 9.718 ms, cached-vs-PRT-tiled speedup 0.892x
seed 23: atlas pass true, max tile count 53, overflow 0, cached 11.036 ms, valid PRT tiled 9.748 ms, cached-vs-PRT-tiled speedup 0.883x
seed 31: atlas pass true, max tile count 48, overflow 0, cached 10.934 ms, valid PRT tiled 9.164 ms, cached-vs-PRT-tiled speedup 0.838x
seed 47: atlas pass true, max tile count 55, overflow 0, cached 10.663 ms, valid PRT tiled 9.295 ms, cached-vs-PRT-tiled speedup 0.872x
```

Validity:

```text
Cached atlas: pass true on all four seeds; cached-vs-scan max_abs 0; direct-dense PSNR 92.56/91.90/91.45/91.14 dB; atlas overflow 0.
Valid PRT tiled: pass true on all four seeds; overflow 0; tiled-vs-dense max_abs <= 1.52e-6.
128-tube regression smoke after the candidate bump: pass true, cached-vs-scan max_abs 0.
```

Read: the larger local candidate store fixes the 256-tube correctness boundary,
but it is not the speed answer. Cached atlas remains faster than scan atlas,
dense direct, and PRT direct, but it is 12%-19% slower than valid PRT tiled at
`4x4x4:256`. This points away from a simple larger local array and toward a
more structured candidate strategy: streamed blocks, band/window split passes,
or a selection scheme that avoids sorting all possible candidates per pixel.

Gate F1h reruns the F1c scan-vs-cached timing probe at 256 tubes with cached
atlas cap64. This isolates whether the F1g loss is a general slowdown or
specifically a loss against valid PRT tiled.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 STAR_UVT_TILE_X=4 STAR_UVT_TILE_Y=4 STAR_UVT_TILE_T=4 STAR_UVT_TILE_CAPACITY=64 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_metal_render_timing_probe.py --seed <17|23|31|47> --target-size 64 --frames 16 --tubes 256 --depth-bands 4 --residual-degree 3 --prt-degree 2 --tile-size 4 --tile-t 4 --support-scale 1.4 --tile-capacity 64 --warmup 2 --iters 5 --min-speedup-gate 1.0 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_metal_render_timing_probe_f1h_64_16f_256t_seed<seed>_tile4_t4_support14_cap64.json
```

Result:

```text
seed 17: pass true, cached-vs-scan max_abs 0, PSNR 120.0 dB, scan 11.200 ms, cached 10.499 ms, cached-vs-scan speedup 1.067x
seed 23: pass true, cached-vs-scan max_abs 0, PSNR 120.0 dB, scan 15.468 ms, cached 10.049 ms, cached-vs-scan speedup 1.539x
seed 31: pass true, cached-vs-scan max_abs 0, PSNR 120.0 dB, scan 13.255 ms, cached 9.516 ms, cached-vs-scan speedup 1.393x
seed 47: pass true, cached-vs-scan max_abs 0, PSNR 120.0 dB, scan 14.453 ms, cached 9.752 ms, cached-vs-scan speedup 1.482x
```

Read: cached cap64 is still better than the scan renderer at 256 tubes, so the
F1g speed loss is specifically against valid PRT tiled. Bigger local arrays
solve correctness but not final speed. The next implementation should reduce
all-candidate list work enough to beat PRT tiled: streamed candidate blocks,
band/window split passes, or a bounded candidate strategy that avoids sorting
every possible candidate per pixel.

Gate F1i tests the simplest exact way to avoid insertion-sort writes:
cache candidate ids/depths unsorted, then repeatedly select the next depth
during compositing. This preserves the same sorted depth order, but still does
quadratic comparisons.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 STAR_UVT_TILE_X=4 STAR_UVT_TILE_Y=4 STAR_UVT_TILE_T=4 STAR_UVT_TILE_CAPACITY=64 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_cached_select_timing_probe.py --seed <17|23|31|47> --target-size 64 --frames 16 --tubes 256 --depth-bands 4 --residual-degree 3 --prt-degree 2 --tile-size 4 --tile-t 4 --support-scale 1.4 --tile-capacity 64 --warmup 2 --iters 5 --min-select-speedup-gate 1.0 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_cached_select_timing_probe_f1i_64_16f_256t_seed<seed>_tile4_t4_support14_cap64.json
```

Result:

```text
seed 17: pass false, select-vs-cached max_abs 0, cached 14.558 ms, select 22.546 ms, scan 21.542 ms, select-vs-cached speedup 0.646x, select-vs-scan speedup 0.955x
seed 23: pass false, select-vs-cached max_abs 0, cached 13.338 ms, select 19.346 ms, scan 15.507 ms, select-vs-cached speedup 0.689x, select-vs-scan speedup 0.802x
seed 31: pass false, select-vs-cached max_abs 0, cached 14.725 ms, select 21.777 ms, scan 14.769 ms, select-vs-cached speedup 0.676x, select-vs-scan speedup 0.678x
seed 47: pass false, select-vs-cached max_abs 0, cached 14.503 ms, select 20.930 ms, scan 20.291 ms, select-vs-cached speedup 0.693x, select-vs-scan speedup 0.969x
```

Read: cached-select is an exact negative control. It avoids insertion-sort
writes, but the repeated selection loop is slower than the existing
insertion-cached path on every seed. Since insertion-cached already loses to
valid PRT tiled at 256 tubes, this path does not need a valid-PRT comparison.
Do not promote cached-select; the next speed attempt needs to reduce candidate
volume or stream bounded chunks, not reshuffle the same full candidate set.

Gate F1j dials the atlas support scale at 256 tubes/cap64. This is not an
exact-coverage promotion; it asks whether a lower support can be a deliberate
speed-mode row while keeping dense-reference error small.

Seed-17 support sweep:

```text
support 1.00: pass false, direct-dense PSNR 67.67 dB, max_abs 0.01633, max tile count 41, cached 15.460 ms, valid PRT tiled 31.328 ms, cached-vs-PRT speedup 2.026x
support 1.10: pass false, direct-dense PSNR 73.34 dB, max_abs 0.01038, max tile count 44, cached 24.087 ms, valid PRT tiled 10.183 ms, cached-vs-PRT speedup 0.423x
support 1.20: pass true, direct-dense PSNR 80.57 dB, max_abs 0.00683, max tile count 47, cached 8.769 ms, valid PRT tiled 11.080 ms, cached-vs-PRT speedup 1.264x
support 1.25: pass true, direct-dense PSNR 83.02 dB, max_abs 0.00559, max tile count 48, cached 8.568 ms, valid PRT tiled 10.073 ms, cached-vs-PRT speedup 1.176x
support 1.30: pass true, direct-dense PSNR 90.61 dB, max_abs 0.00331, max tile count 48, cached 11.866 ms, valid PRT tiled 10.192 ms, cached-vs-PRT speedup 0.859x
support 1.40: pass true, direct-dense PSNR 92.56 dB, max_abs 0.00331, max tile count 52, cached 10.263 ms, valid PRT tiled 8.760 ms, cached-vs-PRT speedup 0.854x
```

Four-seed support 1.25 row:

```text
seed 17: pass true, direct-dense PSNR 83.02 dB, max_abs 0.00559, max tile count 48, cached 8.568 ms, valid PRT tiled 10.073 ms, cached-vs-PRT speedup 1.176x
seed 23: pass true, direct-dense PSNR 85.84 dB, max_abs 0.00553, max tile count 47, cached 8.432 ms, valid PRT tiled 9.859 ms, cached-vs-PRT speedup 1.169x
seed 31: pass true, direct-dense PSNR 84.00 dB, max_abs 0.00559, max tile count 44, cached 7.585 ms, valid PRT tiled 9.305 ms, cached-vs-PRT speedup 1.227x
seed 47: pass true, direct-dense PSNR 83.41 dB, max_abs 0.00576, max tile count 48, cached 9.289 ms, valid PRT tiled 9.991 ms, cached-vs-PRT speedup 1.076x
```

Read: F1j recovers the first four-seed 256-tube cached-atlas speed win against
valid PRT tiled, but only by leaving exact dense coverage. Treat support 1.25
as an explicit speed-mode candidate, not the fidelity default. The exact-ish
support 1.4 row remains the fidelity reference and still loses to valid PRT
tiled; support 1.25 needs downstream quality checks before it can influence
training/playback defaults.

Gate F1k repeats the 256-tube/cap64 comparison at 128px. This is a
higher-resolution synthetic forward stress check, not an overfit or training
quality verdict.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_probe.py --seed <17|23|31|47> --target-size 128 --frames 16 --tubes 256 --depth-bands 4 --residual-degree 3 --prt-degree 2 --atlas-tile-config 4x4x4:64 --prt-tile-config 4x4x4:256 --support-scale <1.25|1.4> --warmup 1 --iters 3 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_f1k_128_seed<seed>_support<support>_16f_256t_atlas4x4x4cap64_prt4x4x4cap256.json
```

Result:

```text
seed 17 support 1.25: pass true, direct-dense PSNR 81.26 dB, max_abs 0.00850, max tile count 42, cached 6.185 ms, valid PRT tiled 8.318 ms, cached-vs-PRT speedup 1.345x
seed 17 support 1.40: pass true, direct-dense PSNR 92.07 dB, max_abs 0.00338, max tile count 47, cached 8.425 ms, valid PRT tiled 8.242 ms, cached-vs-PRT speedup 0.978x
seed 23 support 1.25: pass true, direct-dense PSNR 83.25 dB, max_abs 0.00527, max tile count 37, cached 6.469 ms, valid PRT tiled 8.034 ms, cached-vs-PRT speedup 1.242x
seed 23 support 1.40: pass true, direct-dense PSNR 92.17 dB, max_abs 0.00418, max tile count 47, cached 9.129 ms, valid PRT tiled 7.354 ms, cached-vs-PRT speedup 0.806x
seed 31 support 1.25: pass true, direct-dense PSNR 81.62 dB, max_abs 0.00970, max tile count 35, cached 6.567 ms, valid PRT tiled 7.422 ms, cached-vs-PRT speedup 1.130x
seed 31 support 1.40: pass true, direct-dense PSNR 92.73 dB, max_abs 0.00349, max tile count 39, cached 9.310 ms, valid PRT tiled 7.237 ms, cached-vs-PRT speedup 0.777x
seed 47 support 1.25: pass true, direct-dense PSNR 81.54 dB, max_abs 0.00709, max tile count 45, cached 7.374 ms, valid PRT tiled 7.666 ms, cached-vs-PRT speedup 1.040x
seed 47 support 1.40: pass true, direct-dense PSNR 91.64 dB, max_abs 0.00369, max tile count 52, cached 9.269 ms, valid PRT tiled 7.852 ms, cached-vs-PRT speedup 0.847x
```

Read: the F1j split survives the 128px stress row. Support 1.25 remains a
speed-mode candidate, beating valid PRT tiled by 1.04x-1.34x, but its
direct-dense PSNR drops to 81.3-83.2 dB. Support 1.4 remains the fidelity row
at 91.6-92.7 dB but loses or roughly ties valid PRT tiled. Do not promote
support 1.25 as a default from these synthetic rows alone; the next required
gate is actual overfit/training-quality impact.

Gate F1l audits the existing tracked single-video overfit artifacts before
running new work. C5/C5c already provide screen-time PRT overfit evidence:

```text
64px 4f 200-step comparable-parameter row: PRT 26.4199 dB, train wall 4.528 s, render 8.759 ms; direct-screen baseline 25.0850 dB, train wall 33.081 s, dense-eval render 7.003 ms.
64px 4f same-wall PRT-only row: PRT 28.7288 dB, train wall 36.634 s, render 7.757 ms.
128px 4f 200-step comparable-parameter row: PRT 26.8495 dB, train wall 5.564 s, render 11.024 ms; direct-screen baseline 25.4972 dB, train wall 48.932 s, dense-eval render 23.295 ms.
128px 4f same-wall PRT-only row: PRT 29.3284 dB, train wall 54.291 s, render 10.576 ms.
```

Read: those artifacts are useful PRT-overfit evidence, but they do not test the
inverse-homography atlas-residual `support_scale` policy from F1j/F1k. The
atlas-residual renderer currently has forward Metal paths only; there is no
atlas-residual backward/training harness, so support 1.25 cannot honestly be
called training-quality tested. The next promotion gate is either an atlas
backward/training slice or a clearly named forward-only video-quality stress
row. Do not merge support 1.25 into defaults based on C5/C5c.

Gate F1m tests whether the F1j/F1k support-policy split scales to 512 tubes
without changing the fixed local candidate-store budget. The atlas side keeps
`band_count * tile_capacity <= 256` while valid PRT tiled uses `4x4x4:512`.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_probe.py --seed 17 --target-size 64 --frames 16 --tubes 512 --depth-bands <4|2|1> --residual-degree 3 --prt-degree 2 --atlas-tile-config <4x4x4:64|4x4x4:128|4x4x4:256> --prt-tile-config 4x4x4:512 --support-scale <1.25|1.4> --warmup 1 --iters 3 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_f1m_64_seed17_support<support>_16f_512t_<band-tag>atlas4x4x4cap<cap>_prt4x4x4cap512.json
```

Result:

```text
bands support pass  atlas_pass  PSNR(dB)  max_abs   max_tile  overflow_tiles  cached_ms  prt_ms   cached-vs-PRT
4     1.25    false false       57.54     0.08581   83        48              24.548     29.888   1.218x
4     1.40    false false       52.19     0.11125   91        80              29.988     29.810   0.994x
2     1.25    false false       67.42     0.03650   154       21              21.184     29.716   1.403x
2     1.40    false false       53.54     0.12898   171       31              26.910     29.943   1.113x
1     1.25    false false       77.99     0.00351   235       0               17.662     29.926   1.694x
1     1.40    false false       77.94     0.00351   261       7               22.159     29.745   1.342x
```

Read: seed 17 is already enough to reject this scaling candidate, so there is
no reason to spend a four-seed sweep on the same policy. Multi-band rows keep
the local arrays small but overflow atlas tiles and lose too much image quality.
The single-band cap256/support1.25 row avoids overflow and is fast, but misses
the 80 dB direct-dense quality gate. The 512-tube path needs a different
candidate strategy: exact streamed chunks, split passes with accumulation, or
explicitly larger candidate storage if the memory tradeoff is acceptable.

Gate F1n tests that explicit larger candidate-storage branch. The code now
accepts `STAR_ATLAS_MAX_PIXEL_CANDIDATES=512`, while preserving the 256 default.
This is not a memory-free win; it is a conscious per-pixel local-array budget
increase for atlas cached rendering.

Command:

```text
(cd variants/star_uvt_prt_v0 && python3 setup.py build_ext --inplace)
PYTHONPATH=variants/star_uvt_prt_v0 python3 variants/star_uvt_prt_v0/tests/projective_rational_tile_config_check.py
PYTHONPATH=variants/star_uvt_prt_v0 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_probe.py --seed <17|23|31|47> --target-size 64 --frames 16 --tubes 512 --depth-bands <2|4> --residual-degree 3 --prt-degree 2 --atlas-tile-config <4x4x4:256|4x4x4:128> --prt-tile-config 4x4x4:512 --support-scale 1.25 --atlas-max-pixel-candidates 512 --warmup 1 --iters 3 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_f1n_64_seed<seed>_support1.25_16f_512t_<2band|4band>_atlas4x4x4cap<256|128>_candidate512_prt4x4x4cap512.json
```

Seed-17 boundary rows:

```text
4band support1.40 cap128 candidate512: pass true, direct-dense PSNR 93.95 dB, max_abs 0.00313, max tile count 91, overflow 0, cached 42.496 ms, valid PRT tiled 30.313 ms, cached-vs-PRT speedup 0.713x
4band support1.25 cap128 candidate512: pass true, direct-dense PSNR 84.22 dB, max_abs 0.00566, max tile count 83, overflow 0, cached 28.593 ms, valid PRT tiled 29.937 ms, cached-vs-PRT speedup 1.047x
2band support1.25 cap256 candidate512: pass true, direct-dense PSNR 81.31 dB, max_abs 0.00751, max tile count 154, overflow 0, cached 23.843 ms, valid PRT tiled 29.836 ms, cached-vs-PRT speedup 1.251x
```

Four-seed 4band/support1.25 row:

```text
seed 17: pass true, direct-dense PSNR 84.22 dB, max_abs 0.00566, max tile count 83, overflow 0, cached 28.593 ms, valid PRT tiled 29.937 ms, cached-vs-PRT speedup 1.047x
seed 23: pass true, direct-dense PSNR 85.12 dB, max_abs 0.00519, max tile count 89, overflow 0, cached 27.513 ms, valid PRT tiled 33.129 ms, cached-vs-PRT speedup 1.204x
seed 31: pass true, direct-dense PSNR 84.00 dB, max_abs 0.00491, max tile count 81, overflow 0, cached 24.789 ms, valid PRT tiled 30.807 ms, cached-vs-PRT speedup 1.243x
seed 47: pass true, direct-dense PSNR 82.36 dB, max_abs 0.00770, max tile count 79, overflow 0, cached 30.936 ms, valid PRT tiled 32.426 ms, cached-vs-PRT speedup 1.048x
```

The faster 2band/support1.25 row is not robust:

```text
seed 17: pass true, direct-dense PSNR 81.31 dB, cached-vs-PRT speedup 1.251x
seed 23: pass true, direct-dense PSNR 82.71 dB, cached-vs-PRT speedup 1.317x
seed 31: pass false, direct-dense PSNR 78.95 dB, cached-vs-PRT speedup 1.391x
seed 47: pass true, direct-dense PSNR 81.28 dB, cached-vs-PRT speedup 1.234x
```

Read: candidate512 is a viable 512-tube atlas-cache speed-mode budget. It
clears the F1m overflow/quality failure and recovers a four-seed speed win, but
only with 4 bands at support 1.25. The 2-band row is faster but fails seed 31,
and support 1.4 is the high-fidelity row but loses badly to valid PRT tiled.
Keep `STAR_ATLAS_MAX_PIXEL_CANDIDATES=512` explicit; do not silently change the
default until the memory and 128px stress rows are measured.

Gate F1o stress-tests the F1n candidate512 policy at 128px before any promotion.
This is a seed-17 boundary check, not a four-seed promotion sweep, because the
first rows already fail the speed/quality combination needed for promotion.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_probe.py --seed 17 --target-size 128 --frames 16 --tubes 512 --depth-bands 4 --residual-degree 3 --prt-degree 2 --atlas-tile-config 4x4x4:128 --prt-tile-config 4x4x4:512 --support-scale <1.25|1.20> --atlas-max-pixel-candidates 512 --warmup 1 --iters 3 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_valid_prt_tiled_compare_f1o_128_seed17_support<support>_16f_512t_4band_atlas4x4x4cap128_candidate512_prt4x4x4cap512.json
```

Result:

```text
support 1.25: pass false, atlas pass true, direct-dense PSNR 82.49 dB, max_abs 0.00637, max tile count 67, overflow 0, cached 28.525 ms, PRT tiled 24.860 ms, cached-vs-PRT speedup 0.872x
support 1.20: pass false, atlas pass false, direct-dense PSNR 76.69 dB, max_abs 0.00906, max tile count 64, overflow 0, cached 25.614 ms, PRT tiled 25.399 ms, cached-vs-PRT speedup 0.992x
```

Read: support1.25 remains a valid atlas-quality row at 128px but loses speed to
PRT tiled. Tightening support to 1.20 almost ties timing but drops below the 80
dB gate. The PRT tiled comparator itself misses the strict max-abs pass by a
tiny margin on this seed (`6.4e-05` versus the `5e-05` gate), but it is still
faster with 118 dB direct-dense PSNR, so the atlas path has no promotion case
...
Gate F1r adds the first atlas-residual overfit-quality gate. This deliberately
uses a CPU/direct differentiable render path with fixed atlas compiler state,
fixed depth order, and trainable continuous atlas parameters. It is not a Metal
backward result and not a real heldout/direct-splat result.

Command:

```text
PYTHONPATH=variants/star_uvt_prt_v0 python3 variants/star_uvt_prt_v0/research_project/benchmarks/depth_banded_homography_flow_atlas_cpu_overfit_gate.py --seed 17 --target-size 32 --frames 4 --tubes 64 --steps 200 --depth-bands 4 --residual-degree 3 --tile-size 4 --tile-t 4 --tile-capacity 64 --support-scale 1.4 --out-json variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_cpu_overfit_gate_f1r_seed17_32_4f_64t_200step.json --contact-sheet variants/star_uvt_prt_v0/research_project/benchmarks/results/depth_banded_homography_flow_atlas_cpu_overfit_gate_f1r_seed17_32_4f_64t_200step.png
```
...
```text
pass: true
initial/final loss: 0.00321263 -> 0.00003153
loss drop: 99.02%
initial/final PSNR: 24.93 -> 45.01 dB
final L1/max_abs: 0.00171 / 0.10032
tile pairs: 620
max tile count: 17
overflow: 0 / capacity 64
contact sheet: depth_banded_homography_flow_atlas_cpu_overfit_gate_f1r_seed17_32_4f_64t_200step.png
```

Read: the atlas-residual representation can train through continuous parameters
on a tiny moving-camera target when rendered directly in PyTorch. That closes
the "does the representation have an overfit-quality path at all?" gap, but
only at the CPU/direct level. The next honest atlas-training gate is a Metal VJP
parity check for the same fixed-support scene, then a Metal autograd overfit
rerun. The real heldout/direct-splat atlas comparison is still missing.

Gate F1s adds the first PRT learnable-camera training gate. It uses bounded
per-frame SE3 residuals around a deliberately perturbed train camera, rebuilds
the camera polynomial and PRT coefficients every step, and sends the compiled
`h_coeff` through the existing Metal PRT autograd path. The footprint shape
gradient is still treated as a compiler approximation when requested, and depth
ordering remains discrete, but the core camera-pose-to-PRT-center path is now
trainable.

Command:

```text
dynaworld/.venv/bin/python dynaworld/third_party/fast-mac-gsplat/variants/star_uvt_prt_v0/research_project/benchmarks/projective_rational_learnable_camera_gate.py --device mps --frames 4 --height 32 --width 32 --tube-count 64 --camera-steps 40 --joint-steps 40 --seed 17 --camera-lr 0.03 --world-lr 0.02 --render-warmups 1 --render-repeats 3 --tile-config auto --out-json dynaworld/third_party/fast-mac-gsplat/variants/star_uvt_prt_v0/research_project/benchmarks/results/projective_rational_learnable_camera_gate_f1s_seed17_32_4f_64t_40step.json --out-png dynaworld/third_party/fast-mac-gsplat/variants/star_uvt_prt_v0/research_project/benchmarks/results/projective_rational_learnable_camera_gate_f1s_seed17_32_4f_64t_40step.png
```
...
```text
pass: true
wrong-camera PSNR: 36.69 dB
camera-only loss: 0.00021428 -> 0.00000240
camera-only PSNR: 36.69 -> 56.19 dB
camera grad max: rotation 0.001246, translation 0.000390
joint loss: 0.00092981 -> 0.00008433
joint train/heldout PSNR: 40.74 / 37.64 dB
median joint train render: 50.48 ms
max tile count / overflow: 45 / 0
```

Read: no new math was required for the first learnable-camera PRT proof. Camera
pose residuals can be ordinary bounded parameters as long as the differentiable
camera compiler is kept in the graph before the Metal PRT render. This is still
a synthetic local gate, not the real DeepView dual-camera comparison and not the
atlas Metal-backward path.

Gate F1t forks the real multicam PRT-vs-splat harness into a PRT-only real-clip
trainer. It keeps `load_multicam_video_bundle`, initializes trainable world
tubes from actual clip pixels, trains those world tubes plus bounded train-camera
SE3 residuals, evaluates fixed heldout cameras, writes local MP4 render/GT
videos, and can log the same videos to W&B with `--wandb-enabled`.

Tracked command:

```text
dynaworld/.venv/bin/python dynaworld/third_party/fast-mac-gsplat/variants/star_uvt_prt_v0/research_project/benchmarks/projective_rational_multicam_learnable_camera_train.py --device mps --target-size 64 --max-frames 4 --prt-tubes 128 --steps 20 --seed 17 --world-lr 0.02 --camera-lr 0.03 --render-warmups 1 --render-repeats 3 --out-json dynaworld/third_party/fast-mac-gsplat/variants/star_uvt_prt_v0/research_project/benchmarks/results/projective_rational_multicam_learnable_camera_train_f1t_seed17_64_4f_128t_20step.json --out-dir dynaworld/third_party/fast-mac-gsplat/variants/star_uvt_prt_v0/research_project/benchmarks/results/projective_rational_multicam_learnable_camera_train_f1t_seed17_64_4f_128t_20step_media
```
...
train sampled loss: 0.050244 -> 0.045255
camera grad max: rotation 0.003172, translation 0.010334
eval train/heldout PSNR: 14.23 / 14.18 dB
median train/heldout render: 35.7 / 32.9 ms
max tile count / overflow: 87 / 0
...
representation quality. The next row should scale to the D4b comparison budget
and include the direct-splat baseline again.

Gate F1u fixes the W&B scalar logging gap from F1t and reruns a stronger real
clip overfit row: 1024 PRT world tubes, 200 sequence-loss steps, train-speed
support policy, and the same real DeepView train/heldout camera split. W&B now
gets a per-step `Train/Loss` curve plus final PSNR and media.

Tracked command:

```text
dynaworld/.venv/bin/python dynaworld/third_party/fast-mac-gsplat/variants/star_uvt_prt_v0/research_project/benchmarks/projective_rational_multicam_learnable_camera_train.py --device mps --target-size 64 --max-frames 4 --prt-tubes 1024 --steps 200 --seed 31 --loss-mode sequence --prt-tile-policy train_speed --world-lr 0.02 --camera-lr 0.03 --log-every 10 --render-warmups 1 --render-repeats 3 --wandb-enabled --wandb-mode offline --wandb-project dynaworld --wandb-run-name star-uvt-prt-f1u-real-clip-overfit-seed31-64-4f-1024t-200step-losscurve --wandb-tags star-uvt-prt,real-clip,learnable-camera,f1u,loss-curve,overfit --out-json dynaworld/third_party/fast-mac-gsplat/variants/star_uvt_prt_v0/research_project/benchmarks/results/projective_rational_multicam_learnable_camera_train_f1u_seed31_64_4f_1024t_200step_sequence_wandb.json --out-dir dynaworld/third_party/fast-mac-gsplat/variants/star_uvt_prt_v0/research_project/benchmarks/results/projective_rational_multicam_learnable_camera_train_f1u_seed31_64_4f_1024t_200step_sequence_wandb_media
```
...
train cameras: camera_0006, camera_0014
heldout camera: camera_0005
sequence loss: 0.053215 -> 0.016569
eval train/heldout PSNR: 18.04 / 14.18 dB
median train/heldout render: 16.9 / 10.9 ms
max tile count / overflow: 299 / 0
learned camera residual max: 3.32 deg, 0.097 m
W&B: https://wandb.ai/nbardy/dynaworld/runs/8svxur2o
media: train_render.mp4, train_gt_render.mp4, heldout_render.mp4, heldout_gt_render.mp4
```

Read: F1u is the first real-clip learnable-camera world-tube run with useful
hosted diagnostics. It fits materially better than F1t, and the W&B run now has
a real loss curve, but the visual quality is still not strong: 18 dB train PSNR
at 64px/4 frames is an overfit sanity check, not a win. Heldout stays around
14 dB, so the next useful quality question is still a side-by-side against the
direct dynamic-splat trainer under the same real-clip budget.
