...
@dataclass(frozen=True)
class ProjectiveRationalBackwardProfileResult:
    grad_h_coeff: Tensor
    grad_lambda_uv: Tensor
    grad_lambda_t: Tensor
    grad_center_t: Tensor
    grad_opacity: Tensor
    grad_color: Tensor
    tile_counts: Tensor
    tile_overflow: Tensor
...
@dataclass(frozen=True)
class ProjectiveRationalFusedMSEResult:
    grad_h_coeff: Tensor
    grad_lambda_uv: Tensor
    grad_lambda_t: Tensor
    grad_center_t: Tensor
...
@dataclass(frozen=True)
class AtlasTileAssignmentResult:
    tile_counts: Tensor
    tile_overflow: Tensor
...
def _check_depth_banded_atlas_inputs(
    atlas_ref_uv: Tensor,
    ...
    *,
    ...
    if atlas_ref_uv.ndim != 2 or atlas_ref_uv.shape[-1] != 2:
        raise ValueError("atlas_ref_uv must have shape [N,2]")
    if atlas_residual_coeff.ndim != 3 or atlas_residual_coeff.shape[-1] != 2:
        raise ValueError("atlas_residual_coeff must have shape [N,R,2]")
    if atlas_residual_coeff.shape[1] <= 0:
        raise ValueError("atlas_residual_coeff must contain at least one polynomial term")
    if atlas_ref_uv.shape[0] != atlas_residual_coeff.shape[0]:
        raise ValueError("atlas_ref_uv and atlas_residual_coeff must agree on N")
    ...
    for name, tensor in {
        ...
        "atlas_residual_coeff": atlas_residual_coeff,
        "lambda_uv": lambda_uv,
        "lambda_t": lambda_t,
        "center_t": center_t,
        "opacity": opacity,
    ...
    if assignments.dtype != torch.int32:
        raise ValueError("assignments must be int32")
    if assignments.device != atlas_ref_uv.device:
        raise ValueError("assignments must be on the same device as atlas_ref_uv")
    if not assignments.is_contiguous():
...
def _make_depth_banded_atlas_meta(
    config: UVTRenderConfig,
    device: torch.device,
    tube_count: int,
    *,
    ...
    _runtime_validate(config)
    if atlas_terms <= 0:
        raise ValueError("atlas_terms must be positive")
    if depth_bands <= 0:
        raise ValueError("depth_bands must be positive")
    if support_scale <= 0.0:
        raise ValueError("support_scale must be positive")
    tiles_x = (config.width + config.tile_x - 1) // config.tile_x
    tiles_y = (config.height + config.tile_y - 1) // config.tile_y
    tiles_t = (config.frames + config.tile_t - 1) // config.tile_t
    tile_count = tiles_x * tiles_y * tiles_t * int(depth_bands)
    meta_i32 = torch.tensor(
        [
            config.height,
            config.width,
...
def bin_inverse_homography_atlas_residual_tiles(
    atlas_ref_uv: Tensor,
    atlas_residual_coeff: Tensor,
    lambda_uv: Tensor,
    lambda_t: Tensor,
    ...
    *,
    ...
    tile_counts, tile_overflow, tile_tube_ids = torch.ops.star_uvt_prt_v0.bin_inverse_homography_atlas_residual_tiles(
        atlas_ref_uv,
        atlas_residual_coeff,
        lambda_uv,
        lambda_t,
    ...
    return AtlasTileAssignmentResult(
        tile_counts=tile_counts,
        tile_overflow=tile_overflow,
...
def render_inverse_homography_atlas_residual_tiles_cached(
    atlas_ref_uv: Tensor,
    atlas_residual_coeff: Tensor,
    homographies: Tensor,
    inv_homographies: Tensor,
    ...
    *,
    ...
    image, tile_counts, tile_overflow = torch.ops.star_uvt_prt_v0.render_inverse_homography_atlas_residual_tiles_cached(
        atlas_ref_uv,
        atlas_residual_coeff,
        homographies,
        inv_homographies,
...
def projective_rational_tile_pixel_fused_mse_backward(
    h_coeff: Tensor,
    lambda_uv: Tensor,
    lambda_t: Tensor,
    center_t: Tensor,
    ...
    _check_prt_inputs(h_coeff, lambda_uv, lambda_t, center_t, opacity, color, require_mps=True)
    if h_coeff.shape[1] > 8:
        raise ValueError("fused MSE PRT backward currently supports at most 8 h_coeff terms")
    ...
    (
        ...
        tile_unstable,
        loss_sum,
    ) = torch.ops.star_uvt_prt_v0.projective_rational_tile_pixel_fused_mse_backward(
        h_coeff,
        lambda_uv,
        lambda_t,
        center_t,
...
def profile_projective_rational_tile_pixel_atomic_backward(
    ...
    (
        ...
        tile_unstable,
        timings,
    ) = torch.ops.star_uvt_prt_v0.profile_projective_rational_tile_pixel_atomic_backward(
        h_coeff,
        lambda_uv,
        lambda_t,
        center_t,
    ...
    values = timings.detach().cpu().tolist()
    return ProjectiveRationalBackwardProfileResult(
        grad_h_coeff=grad_h_coeff,
        grad_lambda_uv=grad_lambda_uv,
        grad_lambda_t=grad_lambda_t,
        grad_center_t=grad_center_t,
        ...
        tile_overflow=tile_overflow,
        tile_unstable=tile_unstable,
        timings_ms={key: float(value) for key, value in zip(_PRT_BACKWARD_PROFILE_TIMING_KEYS, values, strict=True)},
    )
