...
profile_projective_rational_tiled_dispatch(
    const torch::Tensor& h_coeff,
    const torch::Tensor& lambda_uv,
    const torch::Tensor& lambda_t,
    const torch::Tensor& center_t,
...
#if defined(__APPLE__)
  if (h_coeff.device().is_mps()) {
    return metal_profile_projective_rational_tiled(
        h_coeff, lambda_uv, lambda_t, center_t, opacity, color, meta_i32, meta_f32);
  }
#endif
  TORCH_CHECK(false, "star_uvt_prt_v0.profile_projective_rational_tiled: no backend available for device ",
              h_coeff.device());
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> bin_inverse_homography_atlas_residual_tiles_dispatch(
  ...
  if (atlas_ref_uv.device().is_mps()) {
    return metal_bin_inverse_homography_atlas_residual_tiles(
        atlas_ref_uv, atlas_residual_coeff, lambda_uv, lambda_t, center_t, opacity, band_ids, meta_i32, meta_f32);
  }
#endif
  TORCH_CHECK(false, "star_uvt_prt_v0.bin_inverse_homography_atlas_residual_tiles: no backend available for device ",
              atlas_ref_uv.device());
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> render_inverse_homography_atlas_residual_tiles_dispatch(
    const torch::Tensor& atlas_ref_uv,
    const torch::Tensor& atlas_residual_coeff,
...
#if defined(__APPLE__)
  if (atlas_ref_uv.device().is_mps()) {
    return metal_render_inverse_homography_atlas_residual_tiles(
...
#endif
  TORCH_CHECK(false,
              "star_uvt_prt_v0.render_inverse_homography_atlas_residual_tiles: no backend available for device ",
              atlas_ref_uv.device());
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> render_inverse_homography_atlas_residual_tiles_cached_dispatch(
    const torch::Tensor& atlas_ref_uv,
    const torch::Tensor& atlas_residual_coeff,
    const torch::Tensor& homographies,
    const torch::Tensor& inv_homographies,
...
#if defined(__APPLE__)
  if (atlas_ref_uv.device().is_mps()) {
    return metal_render_inverse_homography_atlas_residual_tiles_cached(
        atlas_ref_uv, atlas_residual_coeff, homographies, inv_homographies, depth, lambda_uv, lambda_t, center_t,
        opacity, color, band_ids, meta_i32, meta_f32);
  }
#endif
  TORCH_CHECK(false,
              "star_uvt_prt_v0.render_inverse_homography_atlas_residual_tiles_cached: no backend available for "
              "device ",
              atlas_ref_uv.device());
}
...
projective_rational_tile_pair_atomic_backward_dispatch(
    const torch::Tensor& h_coeff,
    const torch::Tensor& lambda_uv,
    const torch::Tensor& lambda_t,
    const torch::Tensor& center_t,
...
#if defined(__APPLE__)
  if (h_coeff.device().is_mps()) {
    return metal_projective_rational_tile_pair_atomic_backward(
        h_coeff, lambda_uv, lambda_t, center_t, opacity, color, grad_image, meta_i32, meta_f32);
  }
#endif
  TORCH_CHECK(false, "star_uvt_prt_v0.projective_rational_tile_pair_atomic_backward: no backend available for device ",
              h_coeff.device());
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
projective_rational_tile_pixel_atomic_backward_dispatch(
    const torch::Tensor& h_coeff,
    const torch::Tensor& lambda_uv,
    const torch::Tensor& lambda_t,
    const torch::Tensor& center_t,
...
#if defined(__APPLE__)
  if (h_coeff.device().is_mps()) {
    return metal_projective_rational_tile_pixel_atomic_backward(
        h_coeff, lambda_uv, lambda_t, center_t, opacity, color, grad_image, meta_i32, meta_f32);
  }
#endif
  TORCH_CHECK(false, "star_uvt_prt_v0.projective_rational_tile_pixel_atomic_backward: no backend available for device ",
              h_coeff.device());
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
           torch::Tensor, torch::Tensor, torch::Tensor>
projective_rational_tile_pixel_fused_mse_backward_dispatch(
    const torch::Tensor& h_coeff,
    const torch::Tensor& lambda_uv,
    const torch::Tensor& lambda_t,
    const torch::Tensor& center_t,
...
#if defined(__APPLE__)
  if (h_coeff.device().is_mps()) {
    return metal_projective_rational_tile_pixel_fused_mse_backward(
        h_coeff, lambda_uv, lambda_t, center_t, opacity, color, target_image, meta_i32, meta_f32);
  }
#endif
  TORCH_CHECK(false,
              "star_uvt_prt_v0.projective_rational_tile_pixel_fused_mse_backward: no backend available for device ",
              h_coeff.device());
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
           torch::Tensor, torch::Tensor, torch::Tensor>
projective_rational_tile_pixel_fused_mse_train_used_backward_dispatch(
    const torch::Tensor& h_coeff,
    const torch::Tensor& lambda_uv,
    const torch::Tensor& lambda_t,
    const torch::Tensor& center_t,
...
#if defined(__APPLE__)
  if (h_coeff.device().is_mps()) {
    return metal_projective_rational_tile_pixel_fused_mse_train_used_backward(
        h_coeff, lambda_uv, lambda_t, center_t, opacity, color, target_image, meta_i32, meta_f32);
  }
#endif
  TORCH_CHECK(false,
              "star_uvt_prt_v0.projective_rational_tile_pixel_fused_mse_train_used_backward: no backend available for device ",
              h_coeff.device());
}
...
TORCH_LIBRARY(star_uvt_prt_v0, m) {
  ...
  m.def("profile_projective_rational_tiled(Tensor h_coeff, Tensor lambda_uv, Tensor lambda_t, Tensor center_t, Tensor opacity, Tensor color, Tensor meta_i32, Tensor meta_f32) -> (Tensor, Tensor, Tensor, Tensor, Tensor)");
  ...
  m.def("projective_rational_tile_pixel_fused_mse_backward(Tensor h_coeff, Tensor lambda_uv, Tensor lambda_t, Tensor center_t, Tensor opacity, Tensor color, Tensor target_image, Tensor meta_i32, Tensor meta_f32) -> (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor)");
  m.def("projective_rational_tile_pixel_fused_mse_train_used_backward(Tensor h_coeff, Tensor lambda_uv, Tensor lambda_t, Tensor center_t, Tensor opacity, Tensor color, Tensor target_image, Tensor meta_i32, Tensor meta_f32) -> (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor)");
...
TORCH_LIBRARY_IMPL(star_uvt_prt_v0, CompositeExplicitAutograd, m) {
  ...
  m.impl("profile_projective_rational_tiled", star_uvt::profile_projective_rational_tiled_dispatch);
  ...
  m.impl("projective_rational_tile_pixel_fused_mse_backward", star_uvt::projective_rational_tile_pixel_fused_mse_backward_dispatch);
  m.impl("projective_rational_tile_pixel_fused_mse_train_used_backward", star_uvt::projective_rational_tile_pixel_fused_mse_train_used_backward_dispatch);
