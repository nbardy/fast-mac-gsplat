...
struct MetalKernels {
  ...
  std::shared_ptr<MetalKernelFunction> render_inverse_homography_atlas_residual_tiles_cached_select;
...
MetalKernels& kernels() {
  ...
  std::call_once(once, []() {
    ...
    out.render_inverse_homography_atlas_residual_tiles_cached_select =
        lib->getKernelFunction("render_inverse_homography_atlas_residual_tiles_cached_select");
    ...
    out.projective_rational_tile_pixel_atomic_backward =
...
void check_float_mps_homographies(const torch::Tensor& t, const char* name, int64_t frames, int64_t bands) {
  TORCH_CHECK(t.device().is_mps(), name, " must be on MPS");
  TORCH_CHECK(t.scalar_type() == torch::kFloat32, name, " must be float32");
  TORCH_CHECK(t.dim() == 4 && t.size(0) == frames && t.size(1) == bands && t.size(2) == 3 && t.size(3) == 3,
              name, " must have shape [frames,bands,3,3]");
  TORCH_CHECK(t.is_contiguous(), name, " must be contiguous");
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> metal_render_inverse_homography_atlas_residual_tiles(
  ...
  TORCH_CHECK(atlas_ref_uv.size(0) == atlas_residual_coeff.size(0) &&
              ...
              "all inverse-homography atlas residual render inputs must agree on N");
...
metal_render_inverse_homography_atlas_residual_tiles_cached_select(
    const torch::Tensor& atlas_ref_uv,
    const torch::Tensor& atlas_residual_coeff,
    const torch::Tensor& homographies,
    const torch::Tensor& inv_homographies,
  ...
  TORCH_CHECK(atlas_ref_uv.size(0) == atlas_residual_coeff.size(0) &&
                  ...
                  atlas_ref_uv.size(0) == center_t.size(0) && atlas_ref_uv.size(0) == opacity.size(0) &&
                  atlas_ref_uv.size(0) == color.size(0) && atlas_ref_uv.size(0) == band_ids.size(0),
              "all cached-select inverse-homography atlas residual render inputs must agree on N");
  ...
  launch(k.render_inverse_homography_atlas_residual_tiles_cached_select, [&](MetalKernelFunction& fn) {
    fn.setArg(0, atlas_ref_uv);
    fn.setArg(1, atlas_residual_coeff);
    fn.setArg(2, homographies);
    fn.setArg(3, inv_homographies);
...
metal_projective_rational_direct_serial_backward(
  ...
  TORCH_CHECK(meta.reserved0 == h_coeff.size(1), "meta reserved0 must equal h_coeff term count");
  TORCH_CHECK(meta.reserved0 > 0, "h_coeff term count must be positive");
  TORCH_CHECK(meta.reserved0 <= 8, "direct serial PRT backward currently supports at most 8 h_coeff terms");
  ...
  launch(k.projective_rational_direct_serial_backward, [&](MetalKernelFunction& fn) {
    fn.setArg(0, h_coeff);
    fn.setArg(1, lambda_uv);
    fn.setArg(2, lambda_t);
    fn.setArg(3, center_t);
    ...
    fn.setArg(7, meta_i32);
    fn.setArg(8, meta_f32);
    fn.setArg(9, grad_h_coeff);
    fn.setArg(10, grad_lambda_uv);
    fn.setArg(11, grad_lambda_t);
    fn.setArg(12, grad_center_t);
    fn.setArg(13, grad_opacity);
    fn.setArg(14, grad_color);
    fn.dispatch((uint64_t)meta.tube_count, 256);
  });
...
metal_projective_rational_tile_pixel_atomic_backward(
  ...
  TORCH_CHECK(meta.reserved0 == h_coeff.size(1), "meta reserved0 must equal h_coeff term count");
  TORCH_CHECK(meta.reserved0 > 0, "h_coeff term count must be positive");
  TORCH_CHECK(meta.reserved0 <= 8, "tile-pixel atomic PRT backward currently supports at most 8 h_coeff terms");
  TORCH_CHECK(grad_image.size(0) == meta.frames && grad_image.size(1) == meta.height && grad_image.size(2) == meta.width,
              "grad_image shape must match meta");
  auto& k = kernels();
  ...
  launch(k.projective_rational_tile_pixel_atomic_backward, [&](MetalKernelFunction& fn) {
    fn.setArg(0, h_coeff);
    fn.setArg(1, lambda_uv);
    fn.setArg(2, lambda_t);
    fn.setArg(3, center_t);
...
metal_projective_rational_tile_pixel_fused_mse_backward_impl(
  ...
  check_float_mps_1d(opacity, "opacity");
  check_float_mps_2d(color, "color", 3);
  TORCH_CHECK(target_image.device().is_mps(), "target_image must be on MPS");
  TORCH_CHECK(target_image.scalar_type() == torch::kFloat32, "target_image must be float32");
  TORCH_CHECK(target_image.dim() == 4 && target_image.size(3) == 3,
              "target_image must have shape [F,H,W,3]");
  TORCH_CHECK(target_image.is_contiguous(), "target_image must be contiguous");
  TORCH_CHECK(h_coeff.size(0) == lambda_uv.size(0) && h_coeff.size(0) == lambda_t.size(0) &&
                  h_coeff.size(0) == center_t.size(0) && h_coeff.size(0) == opacity.size(0) &&
                  h_coeff.size(0) == color.size(0),
              "all PRT inputs must agree on N");
  ...
  TORCH_CHECK(meta.reserved0 == h_coeff.size(1), "meta reserved0 must equal h_coeff term count");
  TORCH_CHECK(meta.reserved0 > 0, "h_coeff term count must be positive");
  TORCH_CHECK(meta.reserved0 <= 8, "fused MSE PRT backward currently supports at most 8 h_coeff terms");
  TORCH_CHECK(target_image.size(0) == meta.frames && target_image.size(1) == meta.height &&
                  target_image.size(2) == meta.width,
              "target_image shape must match meta");
  auto& k = kernels();
  ...
  auto grad_opacity = torch::empty({meta.tube_count}, opts_f);
  auto grad_color = torch::empty({meta.tube_count, 3}, opts_f);
  auto loss_sum = torch::zeros({1}, opts_f);
...
metal_profile_projective_rational_tile_pixel_atomic_backward(
  ...
  auto alloc_tiles_start = Clock::now();
  auto tile_counts = torch::empty({meta.tile_count}, opts_i32);
  auto tile_overflow = torch::empty({meta.tile_count}, opts_i32);
  auto tile_unstable = torch::empty({meta.tile_count}, opts_i32);
  auto tile_tube_ids = torch::empty({meta.tile_count * meta.tile_capacity}, opts_i32);
  auto tile_depths = torch::empty({meta.tile_count * meta.tile_capacity}, opts_f);
  auto alloc_tiles_end = Clock::now();

  auto clear_tiles_start = Clock::now();
  launch(k.clear_tiles, [&](MetalKernelFunction& fn) {
    fn.setArg(0, tile_counts);
    fn.setArg(1, tile_overflow);
    fn.setArg(2, tile_unstable);
  ...
  torch::mps::synchronize();
  auto clear_tiles_end = Clock::now();
  ...
  auto alloc_grads_start = Clock::now();
  auto grad_h_coeff = torch::empty({meta.tube_count, meta.reserved0, 3}, opts_f);
  auto grad_lambda_uv = torch::empty({meta.tube_count, 3}, opts_f);
  auto grad_lambda_t = torch::empty({meta.tube_count}, opts_f);
  auto grad_center_t = torch::empty({meta.tube_count}, opts_f);
  auto grad_opacity = torch::empty({meta.tube_count}, opts_f);
  auto grad_color = torch::empty({meta.tube_count, 3}, opts_f);
  auto alloc_grads_end = Clock::now();

  auto clear_grads_start = Clock::now();
  launch(k.clear_prt_gradients, [&](MetalKernelFunction& fn) {
    fn.setArg(0, grad_h_coeff);
    fn.setArg(1, grad_lambda_uv);
    fn.setArg(2, grad_lambda_t);
  ...
  torch::mps::synchronize();
  auto clear_grads_end = Clock::now();
  ...
  auto debug_sink = torch::empty({entry_count}, opts_f);
  auto compute_only_start = Clock::now();
  launch(k.projective_rational_tile_pixel_compute_only_backward, [&](MetalKernelFunction& fn) {
    fn.setArg(0, h_coeff);
    fn.setArg(1, lambda_uv);
    fn.setArg(2, lambda_t);
    fn.setArg(3, center_t);
  ...
  timing_ptr[2] = elapsed_ms(bin_start, bin_end);
  timing_ptr[3] = elapsed_ms(alloc_grads_start, alloc_grads_end);
