...
class V9HWSortedEvalCapabilities:
    mps_available: bool
...
def _native_probe(compile_pipelines: bool, compile_advanced: bool) -> Dict[str, Any]:
    if _C is None:
        return {
            ...
            "native_probe_error": "native extension is not imported; build variants/v9_hw_sorted_eval_probe first",
        }
...
def render_constant_rgba(height: int, width: int, rgba: RGBA = (0.125, 0.5, 0.875, 1.0)) -> Tensor:
    ...
    if _C is None:
        raise RuntimeError("native extension is not imported; build variants/v9_hw_sorted_eval_probe first")
...
def render_gaussian_eval_rgba_sorted(
    ...
    *,
    ...
    `descending=False` matches the v8 wrapper convention:
    `torch.argsort(depths.detach(), stable=True)`, so lower numeric depths are
    submitted first and equal-depth splats keep their input order. Set
    `descending=True` to submit higher numeric depths first.

    This wrapper only changes draw order. The native v9 eval path still uses
    fixed-function source-over blending, so this is not exact v8 front-to-back
    transmittance parity.
    """
...
def probe_hw_interop(
    *,
    ...
    return V9HWSortedEvalCapabilities(
