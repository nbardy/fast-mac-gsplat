...
from torch_gsplat_bridge_v9_hw_sorted_eval import (
    probe_hw_interop,
...
def main() -> None:
    ...
    two_means = torch.tensor([[8.5, 8.5], [8.5, 8.5]], dtype=torch.float32, device="mps")
    ...
    input_order = render_gaussian_eval_rgba(
        two_means, two_conics, two_colors, two_opacities, 16, 16, direct=True
    )
    reverse_order = render_gaussian_eval_rgba(
    ...
    input_center = input_order.detach().cpu()[8, 8]
    reverse_center = reverse_order.detach().cpu()[8, 8]
    order_delta = float((input_center - reverse_center).abs().max().item())
    assert order_delta > 0.1, order_delta

    depths = torch.tensor([1.0, 0.0], dtype=torch.float32, device="mps")
    sorted_a = render_gaussian_eval_rgba_sorted(
        two_means, two_conics, two_colors, two_opacities, depths, 16, 16, direct=True
    )
    sorted_b = render_gaussian_eval_rgba_sorted(
        two_means, two_conics, two_colors, two_opacities, depths, 16, 16, direct=True
    )
    sorted_repeat_err = float((sorted_a.detach().cpu() - sorted_b.detach().cpu()).abs().max().item())
    assert sorted_repeat_err == 0.0, sorted_repeat_err
    sorted_manual_err = float((sorted_a.detach().cpu() - reverse_order.detach().cpu()).abs().max().item())
    assert sorted_manual_err <= 1.0e-6, sorted_manual_err

    sorted_desc = render_gaussian_eval_rgba_sorted(
        two_means, two_conics, two_colors, two_opacities, depths, 16, 16, direct=True, descending=True
    )
    sorted_desc_err = float((sorted_desc.detach().cpu() - input_order.detach().cpu()).abs().max().item())
    assert sorted_desc_err <= 1.0e-6, sorted_desc_err
