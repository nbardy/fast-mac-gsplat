...
`variants/v6_refined_features_f32_reduce`, created to test 4-channel
dot/reduction fusion without mutating the stable `v6_refined_features` baseline
or the current reduction fork.
...
- Package: `torch_gsplat_bridge_v6_refined_features_f32_block4`
...
- v6_refined_features_f32_block4 reuses per-channel `grad_features` loads for
  the generic dot product and color-gradient reduction in 4-channel blocks. It
  passed correctness checks but regressed all measured F32 timing rows, so keep
  it benchmark-only.
...
- Keep original `v6_refined_features` as the stable v6 F-channel baseline; use
  this fork only for isolated benchmark experiments.
