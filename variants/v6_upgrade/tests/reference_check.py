...
def check_case(B: int, *, active_policy: str, use_active_tiles):
    ...
    out = rasterize_projected_gaussians(
        means2d if B > 1 else means2d[0],
        conics if B > 1 else conics[0],
        colors if B > 1 else colors[0],
        opacities if B > 1 else opacities[0],
        depths if B > 1 else depths[0],
        cfg,
    )
    ...
    mode = f"policy={active_policy},override={use_active_tiles}"
...
def check_saturated_many_splats(*, active_policy: str, use_active_tiles):
    device = torch.device("mps")
...
def main():
    ...
    for active_policy, use_active_tiles in (("off", None), ("on", None), ("auto", False)):
        check_case(1, active_policy=active_policy, use_active_tiles=use_active_tiles)
        check_case(2, active_policy=active_policy, use_active_tiles=use_active_tiles)
        check_saturated_many_splats(active_policy=active_policy, use_active_tiles=use_active_tiles)
