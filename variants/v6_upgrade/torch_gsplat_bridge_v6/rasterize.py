...
class RasterConfig:
    ...
    # Direct-tile kernels are the default. Active scheduling is opt-in or auto.
    use_active_tiles: Optional[bool] = None  # explicit legacy override
...
def _maybe_raise_overflow(tile_counts: Tensor, max_fast_pairs: int) -> None:
    if bool((tile_counts > int(max_fast_pairs)).any().item()):
        raise RuntimeError(
            f"Tile overflow detected with max_fast_pairs={int(max_fast_pairs)}. "
...
class _RasterizeProjectedGaussiansV6(torch.autograd.Function):
    ...
    def forward(
        ...
        if enable_overflow_fallback:
            ...
            _maybe_raise_overflow(tile_counts, max_fast_pairs)
            out = out_fast
...
def _rasterize_chunk_eval(
    ...
    if config.enable_overflow_fallback:
        ...
        _maybe_raise_overflow(tile_counts, int(meta_i32[7].item()))
    return out_fast
...
def profile_projected_gaussians(
    ...
    *,
    ...
    for b0 in range(0, B, chunk_b):
        ...
        band_stats = _band_counts(tile_counts, active_tile_ids, config.stop_count_dense_threshold) if use_active_tiles else {
