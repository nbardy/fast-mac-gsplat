...
std::string load_shader_source() {
  ...
  metalPath = [metalPath stringByAppendingPathComponent:@"gsplat_v6_kernels.metal"];
  NSError* err = nil;
  NSString* src = [NSString stringWithContentsOfFile:metalPath encoding:NSUTF8StringEncoding error:&err];
  TORCH_CHECK(src != nil, "Failed to read gsplat_v6_kernels.metal: ", err.localizedDescription.UTF8String);
