...
class RasterConfig:
    ...
    # save | recompute. "recompute" drops tile metadata from autograd saved
    # tensors and reruns bin + forward-state in backward to recover it exactly.
...
def _runtime_validate(config: RasterConfig, feature_dim: int | None = None) -> RuntimeShaderConfig:
    ...
    if config.backward_state_strategy not in ("save", "recompute"):
        raise ValueError("backward_state_strategy must be one of: save, recompute")
...
def _check_inputs(means2d: Tensor, conics: Tensor, colors: Tensor, opacities: Tensor, depths: Tensor) -> None:
    ...
    if means2d.device.type != "mps":
        raise ValueError("v13a_temporal_recompute_state Metal rasterizer inputs must be on MPS")
...
def _recompute_backward_state(
    means_flat: Tensor,
    conics_flat: Tensor,
    colors_flat: Tensor,
    opacities_flat: Tensor,
    meta_i32: Tensor,
    meta_f32: Tensor,
    ...
    *,
    ...
    tile_counts, tile_offsets, binned_ids = torch.ops.gsplat_metal_v13a_temporal_recompute_state.bin(
    ...
    if bool((tile_counts > max_fast_pairs).any().item()):
        raise RuntimeError(
            f"Tile overflow detected during backward recompute with max_fast_pairs={max_fast_pairs}. "
            "The fixedbin fork only supports no-overflow fast-path rows."
        )

    if use_active_tiles:
        active_tile_ids = _make_active_tile_ids(
            tile_counts, max_fast_pairs, sort_by_count=bool(sort_active_tiles_by_count)
        )
        _, _, tile_stop_counts = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_active_forward_state(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
        ...
        active_tile_ids = torch.empty((0,), device=means_flat.device, dtype=torch.int32)
        _, _, tile_stop_counts = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_fast_forward_state(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
    ...
    return active_tile_ids, tile_counts, tile_offsets, binned_ids, tile_stop_counts
...
class _RasterizeProjectedGaussiansV6RefinedFeatures(torch.autograd.Function):
    ...
    def forward(
        ...
        sort_active_tiles_by_count: bool,
        backward_state_strategy: str,
    ) -> tuple[Tensor, Tensor]:
        if not hasattr(torch.ops, "gsplat_metal_v13a_temporal_recompute_state"):
            raise RuntimeError("gsplat_metal_v13a_temporal_recompute_state custom ops not found. Build the extension first.")
        ...
        if use_active_tiles:
            ...
            out_fast, alpha_fast, tile_stop_counts = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_active_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
            ...
            out_fast, alpha_fast, tile_stop_counts = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_fast_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
        ...
        if backward_state_strategy == "recompute":
            empty_i32 = torch.empty((0,), device=means2d_b.device, dtype=torch.int32)
            saved_active_tile_ids = empty_i32
            saved_tile_counts = empty_i32
            saved_tile_offsets = torch.zeros((1,), device=means2d_b.device, dtype=torch.int32)
            saved_binned_ids = empty_i32
            saved_tile_stop_counts = empty_i32
        elif backward_state_strategy == "save":
            saved_active_tile_ids = active_tile_ids
            saved_tile_counts = tile_counts
            saved_tile_offsets = tile_offsets
            saved_binned_ids = binned_ids
            saved_tile_stop_counts = tile_stop_counts
        else:
            raise ValueError("backward_state_strategy must be one of: save, recompute")
        ...
        ctx.save_for_backward(
            ...
            meta_host_i32,
            meta_host_f32,
            saved_active_tile_ids,
            saved_tile_counts,
            saved_tile_offsets,
            saved_binned_ids,
            saved_tile_stop_counts,
            overflow_tile_ids,
        ...
        ctx.sort_active_tiles_by_count = bool(sort_active_tiles_by_count)
        ctx.backward_state_strategy = backward_state_strategy
        return out, alpha
    ...
    def backward(ctx, grad_features: Tensor | None, grad_alpha: Tensor | None):
        ...
        if ctx.backward_state_strategy == "recompute":
            active_tile_ids, tile_counts, tile_offsets, binned_ids, tile_stop_counts = _recompute_backward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
                ...
                meta_host_i32,
                meta_host_f32,
                use_active_tiles=bool(ctx.use_active_tiles),
                sort_active_tiles_by_count=bool(ctx.sort_active_tiles_by_count),
            )
        ...
        if ctx.use_active_tiles:
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_active_backward_saved(
                grad_fast,
                grad_alpha_fast,
                means_flat,
                conics_flat,
            ...
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_fast_backward_saved(
                grad_fast,
                grad_alpha_fast,
                means_flat,
                conics_flat,
        ...
        if has_overflow:
            ...
            go_means, go_conics, go_colors, go_opacities = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_overflow_backward(
                grad_tiles,
                grad_alpha_tiles,
                means_flat,
                conics_flat,
...
def _rasterize_chunk_eval(
    ...
    if use_active_tiles:
        ...
        out_fast, alpha_fast = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_active_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
        ...
        out_fast, alpha_fast = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_fast_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
        ...
        if train_mode:
            ...
            chunk_out, chunk_alpha = (
                _RasterizeProjectedGaussiansV6RefinedFeatures.apply(
                    ...
                    config.backward_state_strategy,
                )
            )
    ...
    *,
    ...
    for b0 in range(0, B, chunk_b):
        ...
        if run_forward or return_image:
            ...
            if use_active_tiles:
                _, _alpha, stop_counts = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_active_forward_state(
                    m_s,
                    q_s,
                    c_s,
                    o_s,
                ...
                _, _alpha, stop_counts = torch.ops.gsplat_metal_v13a_temporal_recompute_state.render_fast_forward_state(
                    m_s,
                    q_s,
                    c_s,
                    o_s,
