...
class V9HWEvalParityCapabilities:
    mps_available: bool
...
def _native_probe(compile_pipelines: bool, compile_advanced: bool) -> Dict[str, Any]:
    if _C is None:
        return {
            ...
            "native_probe_error": "native extension is not imported; build variants/v9_hw_eval_parity_probe first",
        }
...
def render_constant_rgba(height: int, width: int, rgba: RGBA = (0.125, 0.5, 0.875, 1.0)) -> Tensor:
    ...
    if _C is None:
        raise RuntimeError("native extension is not imported; build variants/v9_hw_eval_parity_probe first")
...
def probe_hw_interop(
    *,
    ...
    return V9HWEvalParityCapabilities(
