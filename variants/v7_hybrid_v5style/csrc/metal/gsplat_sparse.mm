...
static std::string read_shader_source() {
  ...
  metalPath = [metalPath stringByAppendingPathComponent:@"gsplat_v73_sparse.metal"];
  NSError* err = nil;
  NSString* src = [NSString stringWithContentsOfFile:metalPath encoding:NSUTF8StringEncoding error:&err];
  TORCH_CHECK(src != nil, "Failed to read gsplat_v73_sparse.metal: ", err.localizedDescription.UTF8String);
...
torch::Tensor metal_forward_eval(
    const torch::Tensor& means2d,
    const torch::Tensor& conics,
    const torch::Tensor& colors,
    const torch::Tensor& opacities,
  ...
  auto out_cpu = texture_to_torch_cpu(tex, meta.batch_size, meta.height, meta.width);
  return out_cpu.to(means2d.device());
}
