...
class _RasterizeProjectedGaussiansV5Features(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means2d_b: Tensor,
        ...
        if not hasattr(torch.ops, "gsplat_metal_v5_features"):
            raise RuntimeError("gsplat_metal_v5_features custom ops not found. Build the extension first.")
        ...
        tile_counts, tile_offsets, binned_ids = torch.ops.gsplat_metal_v5_features.bin(
            means_flat, conics_flat, colors_flat, opacities_flat, meta_i32, meta_f32
        )
        out_fast, alpha_fast, tile_stop_counts = torch.ops.gsplat_metal_v5_features.render_fast_forward_state(
        ...
        if enable_overflow_fallback:
            ...
            if overflow_tile_ids.numel() > 0:
                overflow_tile_imgs, overflow_tile_alpha = torch.ops.gsplat_metal_v5_features.render_overflow_forward(
                    means_flat,
                    conics_flat,
                    colors_flat,
                    opacities_flat,
    ...
    def backward(ctx, grad_features: Tensor | None, grad_alpha: Tensor | None):
        ...
        g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v5_features.render_fast_backward_saved(
            grad_fast,
            grad_alpha_fast,
            means_flat,
            conics_flat,
        ...
        if ctx.enable_overflow_fallback and overflow_tile_ids.numel() > 0:
            ...
            go_means, go_conics, go_colors, go_opacities = torch.ops.gsplat_metal_v5_features.render_overflow_backward(
                grad_tiles,
                grad_alpha_tiles,
                means_flat,
                conics_flat,
...
def _rasterize_chunk_eval(
    ...
    out_fast, alpha_fast = torch.ops.gsplat_metal_v5_features.render_fast_forward_eval(
    ...
    for b0 in range(0, B, chunk_b):
        ...
        if train_mode:
            ...
            chunk_out, chunk_alpha = (
                _RasterizeProjectedGaussiansV5Features.apply(
                    m,
                    q,
                    c,
                    o,
    ...
    *,
    ...
    for b0 in range(0, B, chunk_b):
        ...
        if run_forward or return_image:
            ...
            _, _alpha, stop_counts = torch.ops.gsplat_metal_v5_features.render_fast_forward_state(
