...
std::string load_shader_source() {
  ...
  metalPath = [metalPath stringByAppendingPathComponent:@"gsplat_v5_features_kernels.metal"];
  NSError* err = nil;
  NSString* src = [NSString stringWithContentsOfFile:metalPath encoding:NSUTF8StringEncoding error:&err];
  TORCH_CHECK(src != nil, "Failed to read gsplat_v5_features_kernels.metal: ", err.localizedDescription.UTF8String);
...
MetalV5FeaturesKernels& kernels() {
  static std::once_flag once;
  static std::unique_ptr<DynamicMetalShaderLibrary> lib;
  static MetalV5FeaturesKernels out;
  std::call_once(once, []() {
    lib = std::make_unique<DynamicMetalShaderLibrary>(load_shader_source());
    out.count_tiles = lib->getKernelFunction("v5_features_count_tiles");
    out.emit_binned_ids = lib->getKernelFunction("v5_features_emit_binned_ids");
    out.tile_fast_forward_eval = lib->getKernelFunction("v5_features_tile_fast_forward_eval");
    out.tile_fast_forward_state = lib->getKernelFunction("v5_features_tile_fast_forward_state");
    out.tile_fast_backward_saved = lib->getKernelFunction("v5_features_tile_fast_backward_saved");
    out.tile_overflow_forward = lib->getKernelFunction("v5_features_tile_overflow_forward");
    out.tile_overflow_backward = lib->getKernelFunction("v5_features_tile_overflow_backward");
  });
  return out;
}
