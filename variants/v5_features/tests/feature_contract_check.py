...
import torch_gsplat_bridge_v5_features as v5_features
...
def check_shapes() -> None:
    ...
    for feature_dim in (1, 3, 4, 8, 16, 32, 64):
        cfg = v5_features.RasterConfig(height=height, width=width, tile_size=16, max_fast_pairs=128)
        means2d, conics, colors, opacities, depths = make_case(2, gaussians, feature_dim, height, width, device, 10 + feature_dim)
        with torch.no_grad():
            out_b, alpha_b = v5_features.rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg)
            out_s, alpha_s = v5_features.rasterize_projected_gaussians(means2d[0], conics[0], colors[0], opacities[0], depths[0], cfg)
...
def check_v5_parity() -> None:
    ...
    cfg_features = v5_features.RasterConfig(height=height, width=width, tile_size=16, max_fast_pairs=256, background=(0.1, 0.2, 0.3))
    with torch.no_grad():
        out_v5 = v5.rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg_v5)
        out_features, _alpha = v5_features.rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg_features)
...
def check_feature_grad(feature_dim: int) -> None:
    ...
    cfg = v5_features.RasterConfig(height=height, width=width, tile_size=16, max_fast_pairs=128)
    out, _alpha = v5_features.rasterize_projected_gaussians(means2d, conics, colors_mps, opacities, depths, cfg)
...
def check_no_nan_smoke() -> None:
    device = torch.device("mps")
    cfg = v5_features.RasterConfig(height=16, width=16, tile_size=16, max_fast_pairs=256)
    for i in range(100):
        means2d, conics, colors, opacities, depths = make_case(1, 24, 32, 16, 16, device, 1000 + i)
        colors.requires_grad_(True)
        out, _alpha = v5_features.rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg)
        loss = out.square().mean()
        loss.backward()
