...
import torch_gsplat_bridge_v6_refined_features_f64_accum64 as v6_refined_features_f64_accum64
...
def check_shapes(*, active_policy: str = "off") -> None:
    ...
    for feature_dim in (1, 3, 4, 8, 16, 32, 64):
        cfg = v6_refined_features_f64_accum64.RasterConfig(
            height=height,
            width=width,
            tile_size=16,
            max_fast_pairs=128,
        ...
        with torch.no_grad():
            out_b, alpha_b = v6_refined_features_f64_accum64.rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg)
            out_s, alpha_s = v6_refined_features_f64_accum64.rasterize_projected_gaussians(means2d[0], conics[0], colors[0], opacities[0], depths[0], cfg)
...
def check_v5_parity(*, active_policy: str = "off") -> None:
    ...
    cfg_features = v6_refined_features_f64_accum64.RasterConfig(
        height=height,
        width=width,
        tile_size=16,
        max_fast_pairs=256,
    ...
    with torch.no_grad():
        ...
        out_features, _alpha = v6_refined_features_f64_accum64.rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg_features)
...
def check_feature_grad(feature_dim: int, *, active_policy: str = "off") -> None:
    ...
    out, _alpha = v6_refined_features_f64_accum64.rasterize_projected_gaussians(means2d, conics, colors_mps, opacities, depths, cfg)
...
def check_no_nan_smoke(*, active_policy: str = "off") -> None:
    ...
    for i in range(100):
        ...
        out, _alpha = v6_refined_features_f64_accum64.rasterize_projected_gaussians(means2d, conics, colors, opacities, depths, cfg)
        loss = out.square().mean()
        loss.backward()
