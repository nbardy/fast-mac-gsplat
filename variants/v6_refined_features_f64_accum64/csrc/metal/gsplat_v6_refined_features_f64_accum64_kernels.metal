...
#endif
#ifndef GSP_LOCAL_ACCUM_CAP
#define GSP_LOCAL_ACCUM_CAP 64u
#endif
...
inline void zero_feature_accum(thread float* acc, const constant MetaI32& mi) {
  uint fdim = feature_dim(mi);
  for (uint f = 0u; f < fdim; ++f) {
    acc[f] = 0.0f;
  }
...
inline void accumulate_weighted_features(
    thread float* acc,
    const device float* colors,
    uint g,
    float w,
  ...
  uint color_base = g * fdim;
  for (uint f = 0u; f < fdim; ++f) {
    acc[f] += w * colors[color_base + f];
  }
}

inline void write_feature_accum_with_tail(
    device float* out_features,
    uint pix,
    const thread float* acc,
    float T,
    const constant MetaI32& mi,
    const constant MetaF32& mf) {
  uint fdim = feature_dim(mi);
  uint base = pix * fdim;
  for (uint f = 0u; f < fdim; ++f) {
    out_features[base + f] = acc[f] + T * bg_at(mf, f);
  }
}
...
kernel void v6_refined_features_f64_accum64_count_tiles(
...
kernel void v6_refined_features_f64_accum64_emit_binned_ids(
    const device float2* means2d [[buffer(0)]],
    const device float* conics [[buffer(1)]],
...
kernel void v6_refined_features_f64_accum64_tile_fast_forward_eval(
  ...
  thread float feature_accum[GSP_LOCAL_ACCUM_CAP];
  bool use_local_accum = feature_dim(mi) <= GSP_LOCAL_ACCUM_CAP;
  ...
  if (valid) {
    if (use_local_accum) {
      zero_feature_accum(feature_accum, mi);
    } else {
      zero_pixel(out_features, pix, mi);
    }
  }
  for (uint chunk_start = 0u; chunk_start < count; chunk_start += GSP_CHUNK) {
    ...
    if (valid && T > mf.transmittance_threshold) {
      for (uint j = 0u; j < chunk_n; ++j) {
        ...
        if (use_local_accum) {
          accumulate_weighted_features(feature_accum, colors, g, w, mi);
        } else {
  ...
  if (valid) {
    if (use_local_accum) {
      write_feature_accum_with_tail(out_features, pix, feature_accum, T, mi, mf);
    } else {
...
kernel void v6_refined_features_f64_accum64_tile_fast_forward_state(
    const device uint* tile_counts [[buffer(0)]],
    const device int* tile_offsets [[buffer(1)]],
...
kernel void v6_refined_features_f64_accum64_tile_fast_backward_saved(
...
kernel void v6_refined_features_f64_accum64_tile_active_forward_eval(
...
kernel void v6_refined_features_f64_accum64_tile_active_forward_state(
    const device uint* active_tile_ids [[buffer(0)]],
    const device uint* tile_counts [[buffer(1)]],
    const device int* tile_offsets [[buffer(2)]],
...
kernel void v6_refined_features_f64_accum64_tile_active_backward_saved(
    const device float* grad_features [[buffer(0)]],
    const device float* grad_alpha [[buffer(1)]],
...
kernel void v6_refined_features_f64_accum64_tile_overflow_forward(
...
kernel void v6_refined_features_f64_accum64_tile_overflow_backward(
