...
// v6 batch-first active-tile fast path
kernel void tile_active_forward_eval(
...
kernel void tile_active_forward_state(
    const device uint* active_tile_ids [[buffer(0)]],
    const device uint* tile_counts [[buffer(1)]],
    const device int* tile_offsets [[buffer(2)]],
...
kernel void tile_active_backward_saved(
    const device float* grad_rgb [[buffer(0)]],
    const device uint* active_tile_ids [[buffer(1)]],
