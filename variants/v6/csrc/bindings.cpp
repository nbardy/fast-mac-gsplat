...
#endif
  TORCH_CHECK(false, "gsplat_metal_v6.bin: no backend available for device ", means2d.device());
}
...
#if defined(__APPLE__)
  if (means2d.device().is_mps()) {
    return metal_render_active_forward_eval(means2d, conics, colors, opacities, meta_i32, meta_f32, active_tile_ids, tile_counts, tile_offsets, binned_ids);
  }
#endif
  TORCH_CHECK(false, "gsplat_metal_v6.render_active_forward_eval: no backend available for device ", means2d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v6.render_fast_forward_eval: no backend available for device ", means2d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v6.render_fast_forward_state: no backend available for device ", means2d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v6.render_fast_backward_saved: no backend available for device ", means2d.device());
}
...
#if defined(__APPLE__)
  if (means2d.device().is_mps()) {
    return metal_render_active_forward_state(means2d, conics, colors, opacities, meta_i32, meta_f32, binned_ids, active_tile_ids, tile_counts, tile_offsets);
  }
#endif
  TORCH_CHECK(false, "gsplat_metal_v6.render_active_forward_state: no backend available for device ", means2d.device());
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> render_active_backward_saved_dispatch(
  ...
  if (means2d.device().is_mps()) {
    return metal_render_active_backward_saved(
        grad_out, means2d, conics, colors, opacities, meta_i32, meta_f32, active_tile_ids, tile_counts, tile_offsets, binned_ids, tile_stop_counts);
  }
#endif
  TORCH_CHECK(false, "gsplat_metal_v6.render_active_backward_saved: no backend available for device ", means2d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v6.render_overflow_forward: no backend available for device ", means2d.device());
}
...
#endif
  TORCH_CHECK(false, "gsplat_metal_v6.render_overflow_backward: no backend available for device ", means2d.device());
}

}  // namespace
}  // namespace gsplat

TORCH_LIBRARY(gsplat_metal_v6, m) {
  ...
  m.def("render_active_forward_eval(Tensor means2d, Tensor conics, Tensor colors, Tensor opacities, Tensor meta_i32, Tensor meta_f32, Tensor active_tile_ids, Tensor tile_counts, Tensor tile_offsets, Tensor binned_ids) -> Tensor");
  m.def("render_active_forward_state(Tensor means2d, Tensor conics, Tensor colors, Tensor opacities, Tensor meta_i32, Tensor meta_f32, Tensor(a!) binned_ids, Tensor active_tile_ids, Tensor tile_counts, Tensor tile_offsets) -> (Tensor, Tensor)");
  m.def("render_active_backward_saved(Tensor grad_out, Tensor means2d, Tensor conics, Tensor colors, Tensor opacities, Tensor meta_i32, Tensor meta_f32, Tensor active_tile_ids, Tensor tile_counts, Tensor tile_offsets, Tensor binned_ids, Tensor tile_stop_counts) -> (Tensor, Tensor, Tensor, Tensor)");
...
TORCH_LIBRARY_IMPL(gsplat_metal_v6, CompositeExplicitAutograd, m) {
