...
Use `benchmarks/benchmark_matrix.py` for exploratory sweeps. Its defaults are
small (`128px`, `G=1024`, `B=1`, `F=32`), sequential, timeout-bounded, and omit
`overflow_stress`. Run `--dry-run` before expanding any matrix.

Treat the matrix below as an opt-in engineering plan, not a default command.
Do not jump to large resolution or overflow stress while the machine is in use.
...
- dense backward-input MiB from `benchmarks/rgb_grad_handoff_accounting.py`
- expected MiB avoided by replacing `grad_features[B,H,W,F] + grad_alpha[B,H,W]`
  with `grad_composed_rgb[B,H,W,3]`
...
## v13b-specific accounting smoke
