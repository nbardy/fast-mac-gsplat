...
std::string load_shader_source() {
  ...
  metalPath = [metalPath stringByAppendingPathComponent:@"gsplat_v6_refined_features_f32_zero_bg_kernels.metal"];
  NSError* err = nil;
  NSString* src = [NSString stringWithContentsOfFile:metalPath encoding:NSUTF8StringEncoding error:&err];
  TORCH_CHECK(src != nil, "Failed to read gsplat_v6_refined_features_f32_zero_bg_kernels.metal: ", err.localizedDescription.UTF8String);
...
MetalV6RefinedFeaturesKernels& kernels() {
  ...
  std::call_once(once, []() {
    ...
    out.count_tiles = lib->getKernelFunction("v6_refined_features_f32_zero_bg_count_tiles");
    out.emit_binned_ids = lib->getKernelFunction("v6_refined_features_f32_zero_bg_emit_binned_ids");
    out.tile_fast_forward_eval = lib->getKernelFunction("v6_refined_features_f32_zero_bg_tile_fast_forward_eval");
    out.tile_fast_forward_state = lib->getKernelFunction("v6_refined_features_f32_zero_bg_tile_fast_forward_state");
    out.tile_fast_backward_saved = lib->getKernelFunction("v6_refined_features_f32_zero_bg_tile_fast_backward_saved");
    out.tile_active_forward_eval = lib->getKernelFunction("v6_refined_features_f32_zero_bg_tile_active_forward_eval");
    out.tile_active_forward_state = lib->getKernelFunction("v6_refined_features_f32_zero_bg_tile_active_forward_state");
    out.tile_active_backward_saved = lib->getKernelFunction("v6_refined_features_f32_zero_bg_tile_active_backward_saved");
    out.tile_overflow_forward = lib->getKernelFunction("v6_refined_features_f32_zero_bg_tile_overflow_forward");
    out.tile_overflow_backward = lib->getKernelFunction("v6_refined_features_f32_zero_bg_tile_overflow_backward");
  });
  return out;
}
