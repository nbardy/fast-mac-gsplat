...
This is an isolated feature-channel namespace fork copied from
`variants/v6_refined_features_f32_reduce`, created so the Dynaworld trainer can
test zero-feature-background tail skipping without mutating the stable
`v6_refined_features` baseline or the existing `f32_reduce` reference fork.
...
- Package: `torch_gsplat_bridge_v6_refined_features_f32_zero_bg`
...
- v6_refined_features_f32_zero_bg uses one generic feature-channel path. It does not stage feature
...
- Keep original `v6_refined_features` and `f32_reduce` as stable comparison
  points; use this fork only when callers explicitly request
  `render.fast_mac.feature_variant = "v6_refined_features_f32_zero_bg"`.
- This fork only skips the final background-tail add when
  `RasterConfig.background` is exactly zero. It still performs explicit
  per-pixel zero/write initialization. Do not replace that path with implicit
  `torch::zeros` allocation without repeated-call parity tests.
