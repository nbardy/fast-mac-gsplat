...
@dataclass(frozen=True)
class FeatureLookupRasterResult:
    features: Tensor
    alpha: Tensor
    compact: Tensor
...
def _zero_compact_background_config(config: RasterConfig, compact_dim: int) -> RasterConfig:
    return replace(config, background=tuple(0.0 for _ in range(compact_dim)))


def _full_feature_background(config: RasterConfig, feature_dim: int, device: torch.device, dtype: torch.dtype) -> Tensor:
    background = tuple(float(v) for v in config.background)
    if len(background) == 1:
        values = [background[0]] * feature_dim
    elif len(background) == feature_dim:
        ...
        raise ValueError(
            "For feature lookup rasterization, RasterConfig.background is interpreted in the "
            f"reconstructed feature space and must have length 1 or output feature_dim={feature_dim}; "
            f"got {len(background)}"
        )
    return torch.tensor(values, device=device, dtype=dtype)


def _check_lookup_inputs(feature_weights: Tensor, feature_lookup: Tensor) -> None:
    if feature_lookup.ndim != 2:
        raise ValueError("feature_lookup must have shape [compact_dim, feature_dim]")
    if feature_weights.ndim not in (2, 3):
        raise ValueError("feature_weights must have shape [G,K] or [B,G,K]")
    if feature_weights.shape[-1] != feature_lookup.shape[0]:
        raise ValueError(
            f"feature_weights compact_dim={feature_weights.shape[-1]} must match "
            f"feature_lookup.shape[0]={feature_lookup.shape[0]}"
        )
    if feature_lookup.shape[1] <= 0:
        raise ValueError("feature_lookup output feature_dim must be positive")
    if feature_weights.device != feature_lookup.device:
        raise ValueError("feature_weights and feature_lookup must be on the same device")
    if feature_weights.dtype != torch.float32 or feature_lookup.dtype != torch.float32:
        raise ValueError("feature_weights and feature_lookup must be float32")


def feature_ids_to_coefficients(feature_ids: Tensor, feature_id_weights: Tensor, compact_dim: int) -> Tensor:
    if feature_ids.shape != feature_id_weights.shape:
        raise ValueError("feature_ids and feature_id_weights must have the same shape")
    if feature_ids.ndim not in (2, 3):
        raise ValueError("feature_ids must have shape [G,L] or [B,G,L]")
    if feature_ids.dtype not in (torch.int32, torch.int64, torch.long):
        raise ValueError("feature_ids must be an integer tensor")
    if feature_id_weights.dtype != torch.float32:
        raise ValueError("feature_id_weights must be float32")
    if compact_dim <= 0:
    ...
    if feature_ids.numel() > 0:
        ...
        if min_id < 0 or max_id >= compact_dim:
            raise ValueError(f"feature_ids must be in [0,{compact_dim}); got min={min_id}, max={max_id}")
    out_shape = (*feature_ids.shape[:-1], compact_dim)
    out = torch.zeros(out_shape, device=feature_id_weights.device, dtype=feature_id_weights.dtype)
    out.scatter_add_(-1, feature_ids.to(device=feature_id_weights.device, dtype=torch.long), feature_id_weights)
    return out
...
def _check_inputs(means2d: Tensor, conics: Tensor, colors: Tensor, opacities: Tensor, depths: Tensor) -> None:
    ...
    if means2d.device.type != "mps":
        raise ValueError("v6_feature_lookup_experiment Metal rasterizer inputs must be on MPS")
...
class _RasterizeProjectedGaussiansV6FeatureLookupExperiment(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means2d_b: Tensor,
        ...
        if not hasattr(torch.ops, "gsplat_metal_v6_feature_lookup_experiment"):
            raise RuntimeError("gsplat_metal_v6_feature_lookup_experiment custom ops not found. Build the extension first.")
        ...
        tile_counts, tile_offsets, binned_ids = torch.ops.gsplat_metal_v6_feature_lookup_experiment.bin(
        ...
        if use_active_tiles:
            ...
            out_fast, alpha_fast, tile_stop_counts = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_active_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
            ...
            out_fast, alpha_fast, tile_stop_counts = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_fast_forward_state(
                means_flat,
                conics_flat,
                colors_flat,
                opacities_flat,
        ...
        if enable_overflow_fallback:
            ...
            if overflow_tile_ids.numel() > 0:
                overflow_tile_imgs, overflow_tile_alpha = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_overflow_forward(
                    means_flat,
                    conics_flat,
                    colors_flat,
                    opacities_flat,
                ...
                out = out_fast.clone()
                alpha = alpha_fast.clone()
                _scatter_tile_images_(out, overflow_tile_ids, overflow_tile_imgs, tiles_per_image, tiles_x, tile_size)
                _scatter_tile_scalars_(alpha, overflow_tile_ids, overflow_tile_alpha, tiles_per_image, tiles_x, tile_size)
            else:
                out = out_fast
                alpha = alpha_fast
        else:
    ...
    def backward(ctx, grad_features: Tensor | None, grad_alpha: Tensor | None):
        ...
        if grad_features is None:
            grad_features = torch.zeros(
                (ctx.batch_size, int(meta_i32[0].item()), int(meta_i32[1].item()), ctx.feature_dim),
                device=means_flat.device,
                dtype=means_flat.dtype,
            )
        if grad_alpha is None:
            grad_alpha = torch.zeros(
                (ctx.batch_size, int(meta_i32[0].item()), int(meta_i32[1].item())),
                device=means_flat.device,
                dtype=means_flat.dtype,
            )

        grad_fast = grad_features.contiguous().clone()
        grad_alpha_fast = grad_alpha.contiguous().clone()
        ...
        if ctx.use_active_tiles:
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_active_backward_saved(
                grad_fast,
                grad_alpha_fast,
                means_flat,
                conics_flat,
            ...
            g_means_flat, g_conics_flat, g_colors_flat, g_opacities_flat = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_fast_backward_saved(
                grad_fast,
                grad_alpha_fast,
                means_flat,
                conics_flat,
        ...
        if ctx.enable_overflow_fallback and overflow_tile_ids.numel() > 0:
            ...
            go_means, go_conics, go_colors, go_opacities = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_overflow_backward(
                grad_tiles,
                grad_alpha_tiles,
                means_flat,
                conics_flat,
        ...
        if ctx.inputs_sorted_by_depth:
            ...
            g_colors_b = g_colors_flat.view(B, G, F)
...
def _rasterize_chunk_eval(
    ...
    B, G = means2d_b.shape[:2]
    F = colors_b.shape[-1]
    meta_i32, meta_f32 = _make_meta(config, means2d_b.device, B, G, F)
    ...
    if use_active_tiles:
        ...
        out_fast, alpha_fast = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_active_forward_eval(
            means_flat,
            conics_flat,
            colors_flat,
            opacities_flat,
        ...
        out_fast, alpha_fast = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_fast_forward_eval(
    ...
    if config.enable_overflow_fallback:
        ...
        if overflow_tile_ids.numel() > 0:
            ...
            _scatter_tile_scalars_(alpha, overflow_tile_ids, overflow_tile_alpha, int(meta_i32[10].item()), int(meta_i32[3].item()), int(meta_i32[4].item()))
            return out, alpha
...
def _rasterize_batched(
    ...
    for b0 in range(0, B, chunk_b):
        ...
        if train_mode:
            meta_i32, meta_f32 = _make_meta(config, m.device, b1 - b0, G, c.shape[-1])
            chunk_out, chunk_alpha = (
                _RasterizeProjectedGaussiansV6FeatureLookupExperiment.apply(
                    m,
                    q,
                    c,
                    o,
...
def rasterize_projected_gaussians_feature_lookup(
    means2d: Tensor,
    conics: Tensor,
    ...
    """Rasterize compact basis coefficients, then reconstruct full features.

    `feature_weights` has shape `[G,K]` or `[B,G,K]`. The copied Metal kernel
    only sees K compact channels. The reconstructed output is
    `compact @ feature_lookup + (1 - alpha) * background`, where
    `feature_lookup` has shape `[K,F]` and `config.background` is interpreted in
    the reconstructed F-dimensional feature space.
    """

    _check_lookup_inputs(feature_weights, feature_lookup)
    compact_config = _zero_compact_background_config(config, int(feature_weights.shape[-1]))
    compact, alpha = rasterize_projected_gaussians(
        means2d,
        conics,
        feature_weights,
        opacities,
    ...
    features = torch.matmul(compact, feature_lookup)
    background = _full_feature_background(config, int(feature_lookup.shape[1]), features.device, features.dtype)
    features = features + (1.0 - alpha).unsqueeze(-1) * background
    return FeatureLookupRasterResult(features=features, alpha=alpha, compact=compact)


def rasterize_projected_gaussians_feature_ids(
    means2d: Tensor,
    conics: Tensor,
    feature_ids: Tensor,
    feature_id_weights: Tensor,
    feature_lookup: Tensor,
    opacities: Tensor,
    depths: Tensor,
    config: RasterConfig,
) -> FeatureLookupRasterResult:
    """Prototype ID/weight API that densifies IDs to compact coefficients first.

    This is intentionally a skeleton for API experiments. It validates whether a
    compact K-channel raster pass plus lookup-table reconstruction fits the
    trainer path, but it does not yet avoid the dense `[G,K]` coefficient tensor.
    """

    if feature_lookup.ndim != 2:
        raise ValueError("feature_lookup must have shape [compact_dim, feature_dim]")
    coeffs = feature_ids_to_coefficients(feature_ids, feature_id_weights, int(feature_lookup.shape[0]))
    return rasterize_projected_gaussians_feature_lookup(
        means2d,
        conics,
        coeffs,
        feature_lookup,
...
def profile_projected_gaussians(
    ...
    *,
    ...
    for b0 in range(0, B, chunk_b):
        ...
        meta_i32, meta_f32 = _make_meta(config, means2d_b.device, b1 - b0, G, F)
        tile_counts, tile_offsets, binned_ids = torch.ops.gsplat_metal_v6_feature_lookup_experiment.bin(
        ...
        if run_forward or return_image:
            ...
            if use_active_tiles:
                _, _alpha, stop_counts = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_active_forward_state(
                    m_s, q_s, c_s, o_s, meta_i32, meta_f32, binned_ids, active_tile_ids, tile_counts, tile_offsets
                )
            else:
                _, _alpha, stop_counts = torch.ops.gsplat_metal_v6_feature_lookup_experiment.render_fast_forward_state(
...
class ProjectedGaussianFeatureLookupRasterizer(torch.nn.Module):
    def __init__(self, config: RasterConfig):
        super().__init__()
        self.config = config
