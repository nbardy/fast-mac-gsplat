...
// Feature lookup experiment status:
// These kernels are the v6_refined_features dense-channel path under an
// isolated namespace. The prototype treats the channel dimension as compact
// basis coefficients K, then reconstructs full F features in Python with a
// lookup table. A true sparse-ID Metal path still needs new kernel signatures
// for feature_ids[BG,L], feature_weights[BG,L], and lookup[K,F].
...
inline void atomic_add_feature_grads(
  ...
  uint color_base = g * fdim;
  for (uint f = 0u; f < fdim; ++f) {
    atomic_fetch_add_explicit(&g_colors[color_base + f], grad_features[grad_base + f] * scale, memory_order_relaxed);
  }
}
...
kernel void v6_feature_lookup_experiment_count_tiles(
...
kernel void v6_feature_lookup_experiment_emit_binned_ids(
    const device float2* means2d [[buffer(0)]],
    const device float* conics [[buffer(1)]],
...
kernel void v6_feature_lookup_experiment_tile_fast_forward_eval(
...
kernel void v6_feature_lookup_experiment_tile_fast_forward_state(
    const device uint* tile_counts [[buffer(0)]],
    const device int* tile_offsets [[buffer(1)]],
...
kernel void v6_feature_lookup_experiment_tile_fast_backward_saved(
...
kernel void v6_feature_lookup_experiment_tile_active_forward_eval(
...
kernel void v6_feature_lookup_experiment_tile_active_forward_state(
    const device uint* active_tile_ids [[buffer(0)]],
    const device uint* tile_counts [[buffer(1)]],
    const device int* tile_offsets [[buffer(2)]],
...
kernel void v6_feature_lookup_experiment_tile_active_backward_saved(
    const device float* grad_features [[buffer(0)]],
    const device float* grad_alpha [[buffer(1)]],
...
kernel void v6_feature_lookup_experiment_tile_overflow_forward(
...
kernel void v6_feature_lookup_experiment_tile_overflow_backward(
    ...
    for (int local = int(chunk_n) - 1; local >= 0; --local) {
      ...
      if (valid && global_i < end_i) {
        ...
        if (eval_alpha(p, m, q.x, q.y, q.z, opacity, mf, alpha, raw_alpha, power, d)) {
          float denom = max(1.0f - alpha, mf.eps);
          float T_prev = T_cur / denom;
          float dot_gc = dot_pixel_features(grad_tiles, colors, pix, g, mi) + alpha_grad;
          float g_alpha = T_prev * (dot_gc - gT);
          atomic_add_feature_grads(g_colors, grad_tiles, pix, g, T_prev * alpha, mi);
          float gate = (raw_alpha < mf.max_alpha) ? 1.0f : 0.0f;
          float g_raw = g_alpha * gate;
          float g_power = g_raw * raw_alpha;
          l_ga = g_power * (-0.5f) * d.x * d.x;
